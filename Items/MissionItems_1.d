...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, " Už jsem ti nękolikrát pâátelsky"				);
					Doc_PrintLine	(nDocID,  0, " pâipomínal, že nutnę potâebuji"				);
					Doc_PrintLine	(nDocID,  0, " ty zbranę, které jsem si"				);
					Doc_PrintLine	(nDocID,  0, " u tebe pâed týdnem"			);
					Doc_PrintLine	(nDocID,  0, " objednal."										);
					Doc_PrintLine	(nDocID,  0, " Jestli mi ve velmi brzké dobę"		);
					Doc_PrintLine	(nDocID,  0, " nepodáš žádnou zprávu, budu se k tobę"			);
...
instance ItMi_MariasGoldPlate (C_Item)
{
	...
	TEXT[2]				=	"Na talíâi jsou vyvedena"; 
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, "Léčivý koâen"    );
					Doc_PrintLine	( nDocID,  0, "Ohnivá kopâiva"  );
					Doc_PrintLine	( nDocID,  0, "Ohnivé býlí"  );
					Doc_PrintLine	( nDocID,  0, "Ohnivý koâen"  );
					Doc_PrintLine	( nDocID,  0, "Gobliní bobule"     );
					Doc_PrintLine	( nDocID,  0, "Dračí koâen");
					Doc_PrintLine	( nDocID,  0, "Chŕapavčí býlí"   );
					Doc_PrintLine	( nDocID,  0, "Luční pohanka"      	);
					Doc_PrintLine	( nDocID,  0, "Královský šăovík"       );
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "Poznámka: Aă ti ty byliny obstará nęjaký schopný učeŕ, aspoŕ poznáš, jestli opravdu za nęco stojí. "						);
					Doc_PrintLine	( nDocID,  0, ""						);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLines	( nDocID,  0, "K pâípravę magických lektvarů potâebuje alchymista:");
					Doc_PrintLine	( nDocID,  0, ""						);
					Doc_PrintLine	( nDocID,  0, "ohnivou kopâivu");
					Doc_PrintLine	( nDocID,  0, "ohnivé býlí");
					Doc_PrintLine	( nDocID,  0, "ohnivý koâen.");
					Doc_PrintLines	( nDocID,  0, "Dále si musí opatâit ještę "						);
...
INSTANCE ItWr_Passierschein	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[3]				=	"S tęmito dokumenty mne";
	TEXT[4]				=   "stráže pustí do męsta.";
};
...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, " se může po neomezenou dobu volnę"		);
...
INSTANCE ItMi_HerbPaket(C_Item)
{
	...
	description			= 	name;
	TEXT[2]				= 	"Tęžký a mękký balíček, který";
...
{	
	...
	description			= 	"Tato ryba vypadá zvláštnę.";
	TEXT[2]				= 	"S touto rybou není nęco v poâádku.";		
	TEXT[3]				= 	"Jako by ji nękdo rozâízl a pak zašil.";
};
...
INSTANCE ItWr_HalvorMessage	(C_Item)
{
	...
	scemeName			=	"MAP";
	description			= 	name;
	TEXT[2]				=   "Tato zpráva byla ukryta v rybę.";
};
...
{   
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, " Stráže začínají mít podezâení."				);
					Doc_PrintLine	(nDocID,  0, " Nedęlejte nic, dokud se vám zase neozvu!");
					Doc_PrintLine	(nDocID,  0, ""													);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Cítím, že tu s námi dlí ještę jedna moc, která je čím dál silnęjší."					);
					Doc_PrintLines	( nDocID,  0, "Je to nęco naprosto neznámého. Že by se k nám blížili Beliarovi vazalové?"					);
					Doc_PrintLines	( nDocID,  0, "Možná se pletu, ale vęâím, že by bylo moudré tuhle vęc svęâit otci Pyrokarovi. "					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Nechă vyslyší naše modlitby."					);
					Doc_PrintLine	( nDocID,  0, ""					);
...
INSTANCE ItKe_ThiefGuildKey_MIS (C_Item)
{
	...
	TEXT[2]				=	"Tento klíč poznamenala slaná moâská voda.";
...
INSTANCE ItKe_MagicChest(C_Item)
{
	...
	TEXT[3]				=   "Mohl by patâit k nęjakému";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Zajistęte mnę i mým mužům bezúhonnost pâed Innosem i pâed králem "				);
					Doc_PrintLines	( nDocID,  0, "a my vám na oplátku pomůžeme ochraŕovat męsto i okolní zemę. " );
					Doc_PrintLines	( nDocID,  0, "Já sám krom toho osobnę dohlédnu, aby se moji muži drželi dál od okolních statků - samozâejmę kromę toho Onarova, kde budeme sídlit i nadále." );
					Doc_PrintLines	( nDocID,  0, "Vím, že na palubę lodi, kterou vypravujete k pevninę, je dost místa pro mę i pro nękteré z mých mužů. Proto chci plout s Vámi." );
					Doc_PrintLines	( nDocID,  0, "Prosím Vás, rozhodnęte se moudâe. " );
					Doc_PrintLine	( nDocID,  0, ""								);
...
{   
		...
		if (Bdt13_Dexter_verraten == FALSE)
		{
			...
			B_LogEntry (Topic_Bandits,"Vůdce banditů je Dexter. Skrývá se ve starém dole nedaleko Onarova statku.");
		...
		*/
		...
		nDocID = 	Doc_Create		();
					...
					Doc_PrintLine	(nDocID,  0, " který pâichází z hor."						);
					Doc_PrintLine	(nDocID,  0, "Jestli na nęj narazíš, pak "				);
					Doc_PrintLine	(nDocID,  0, " to bude nejspíš náš starý známý."			);
					Doc_PrintLine	(nDocID,  0, "Dej si pozor, aă mu nenaletíš - "				);
					Doc_PrintLine	(nDocID,  0, " je to nebezpečný čarodęj.   "				);
					Doc_PrintLine	(nDocID,  0, "Buë stále ve stâehu."						);
					Doc_PrintLine	(nDocID,  0, "     "											);
					...
					Doc_PrintLine	(nDocID,  0, " tâicet zlaăáků. "							);
					Doc_PrintLine	(nDocID,  0, "Dalších tâicet dostane ten, "				);
...
INSTANCE ItMi_EddasStatue (C_Item)
{
	...
	TEXT[3]				= 	"požehnej mi, veë mne";
	TEXT[4]				= 	"a chraŕ mne od všeho zlého.";
...
INSTANCE ItKe_Salandril(C_Item)
{
	...
	TEXT[2]				=	"Klíč od truhlice,";
	TEXT[3]				=   "která patâí alchymistovi Salandrilovi.";
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Já mistr Thorben, khorinisský tesaâ, dlužím ctihodnému Lehmarovi 200 zlaăáků.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					Doc_PrintLine	( nDocID,  0, "             Thorben");
					Doc_PrintLine	( nDocID,  0, ""					);	
					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Já Coragon, khorinisský hostinský, dlužím ctihodnému Lehmarovi 150 zlaăáků.");
					Doc_PrintLine	( nDocID,  0, ""					);		
					...
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Já Hanna, khorinisská hostinská, dlužím ctihodnému Lehmarovi 250 zlaăáků.");
					Doc_PrintLine	( nDocID,  1, ""					);
...
INSTANCE ITKE_ORLAN_HOTELZIMMER(C_Item)
{
	...
	TEXT[2]				=	"v hospodę 'U Mrtvé harpyje'.";
	TEXT[3]				=   "";
