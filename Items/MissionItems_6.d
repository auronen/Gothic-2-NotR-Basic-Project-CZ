...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Můj mladý chránęnče, hluboce jsi mę zklamal. Jak to, že jsi vystoupil z lodi a pâitom si nevzal Innosovo oko?"					);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Nejdâív jsem myslel, že jsem mrtev, ale pak jsem se pâesvędčil, že v Ivanových starých kostech pâece jen nęjaká síla zůstala. "					);
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Elixír užívej opatrnę, neb má silné vedlejší účinky a může narušit veškerou manu."					);
...
{	
	...
	B_LogEntry (TOPIC_HallenVonIrdorath,"Deník novice černého mága se zmiŕuje o komnatách s pákami, klíčníkovi, kombinaci spínačů a dalších vęcech. Męl bych si to zapamatovat - jednou by se mi to mohlo hodit."); 
};
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Je mi fuk, jestli ten most pâejdete z druhé strany, nebo ne. Pro mę za mę můžete všichni shnít v pekle!"					);
