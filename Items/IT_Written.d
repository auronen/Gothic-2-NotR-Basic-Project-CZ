...
INSTANCE ItWr_EinhandBuch (C_ITEM)
{
	...
	scemeName				=	"MAP";
	description				= 	"Jižanské umęní obrany";
	
	TEXT[2] 				=  "Kniha o umęní boje";
	TEXT[3] 				=  "jednoručními zbranęmi.";
	TEXT[5]					= 	NAME_Value;
	COUNT[5]				= 	value;
	...
	FUNC VOID UseEinhandBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								
					...
					Doc_SetFont 	( nDocID,  0, FONT_BookHeadline	   			); 	// -1 -> all pages
 					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Jižanské umęní obrany"	);
					Doc_SetFont 	( nDocID,  0, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Jižané na rozdíl od seveâanů spoléhají daleko ménę na fyzickou sílu - kromę obratnosti. V horkém podnebí své domoviny dávají pâednost lehké zbroji, která jim nijak nebrání v pohybu. Proto se i jejich styl boje velmi liší od toho, na který jsme zvyklí u nás."					);
					...
					Doc_SetFont 	( nDocID,  1, FONT_Book	   			); 	// -1 -> all pages
					Doc_PrintLine	( nDocID,  1, ""					);
					Doc_PrintLines	( nDocID,  1, "Snad nejznámęjší fintou, kterou jižané používají, je jednoruční blok a krok vzad. Krokem vzad zmírníte sílu útočníkova úderu a zároveŕ zaujmete lepší pozici pro rychlý protiútok."	);
					Doc_PrintLine	( nDocID,  1, ""					);
...
INSTANCE ItWr_ZweihandBuch (C_ITEM)
{
	...
	FUNC VOID UseZweihandBuch()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Silní válečníci dokáží blokováním a odražením soupeâova meče ztlumit úder a zároveŕ svého soka donutit, aby zcela zmęnil postoj i útočnou taktiku. "					);
