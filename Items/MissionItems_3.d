...
{   
	...
	var int nDocID;
			...
			nDocID = 	Doc_Create		();
						...
						Doc_PrintLine	(nDocID,  0, ""												);
						Doc_PrintLine	(nDocID,  0, ""												);
						Doc_PrintLines	(nDocID,  0, "Nositel tohoto listu může vstoupit do posvátných síní kláštera.");
...
INSTANCE ItKe_CHEST_SEKOB_XARDASBOOK_MIS (C_Item)
{
	...
	description			= 	name;
	TEXT[0]				=   "od bedny na Sekobovę farmę.";
...
{	
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Včera večer se mi stala prapodivná vęc. Pâišel ke mnę jeden z tęch černých zakuklenců, o kterých jsme toho v poslední dobę tolik slyšeli. Nebyl jsem moc nervózní, i když z tajemného návštęvníka pâímo čišelo nęjaké zlo a jeho moc byla skoro hmatatelná. Jeho hlas byl však klidný a pâíjemný - męl jsem pocit, že mu prostę musím vęâit.");
...
INSTANCE ItPo_HealHilda_MIS(C_Item)
{
	...
	description		= 	"Léčí černou horečku";
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					Doc_PrintLines	( nDocID,  0, "Určitę se divíš, jak to, že ti píšeme. Donesli jsme totiž mistru Marlasovi všechny úspory a požádali ho, aby tyto âádky napsal za nás. Pâejeme ti do budoucna jen to nejlepší a jako dárek na rozloučenou ti posíláme obrázek, abys męl na co vzpomínat za dlouhých večerů v klášteâe.");
					Doc_PrintLines	( nDocID,  0, "Mnoho štęstí - snad se ti ta malůvka bude líbit.");
					Doc_PrintLine	( nDocID,  0, "");
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Jestli je ti život milý, tak si sbalíš fidlátka a svůj obchod pâenecháš mnę. Tím si budeme kvit.");				
					Doc_PrintLine	( nDocID,  0, "");
...
INSTANCE ItSe_DiegosTreasure_Mis(C_Item)
{
	...
	description			= 	"Diegovův starý męšec.";
	TEXT[5]				= 	NAME_Value;	COUNT[5]	= value;
};
...
{
	name 				=	"Akcie khorinisských rudných dolů";
...
{   
	...
	{
		B_LogEntry (TOPIC_MinenAnteile,"Ten chlapík, který prodal důlní akcie obchodníkům, se jmenoval Salandril. Najdu ho nejspíš v horní čtvrti Khorinisu, jestli tedy už pâed spravedlností neutekl nękam dál."); 
...
{   
		...
		nDocID = 	Doc_Create		()			  ;							// DocManager 
					...
					Doc_PrintLines	( nDocID,  0, "Ale ne abys z toho dęlal estrádu, jinak tę domobrana zatkne, než se stačíš rozkoukat.");
...
INSTANCE ItWr_VinosKellergeister_Mis (C_ITEM)
{
	...
	FUNC VOID Use_VinosKellergeister_Mis()
	{
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager
					...
					Doc_PrintLine	( nDocID,  0, ""					);
					//Absatz
					Doc_PrintLines	( nDocID,  0, "...šăáva z tęchto hroznů je daleko vydatnęjší, než to, co lze získat v tęchto končinách..."					);
					Doc_PrintLines	( nDocID,  0, "");
