...
func int B_TeachPlayerTalentRunes (var C_NPC slf, var C_NPC oth, var int spell)
{
	...
	B_LogEntry (TOPIC_TalentRunes,"K vytvoâení runy potâebuji svitek s daným kouzlem a potâebné ingredience. S jejich pomocí pak mohu podle runové tabulky na čistém runovém kameni zvolenou runu vytvoâit.");
	...
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy SVĘTLO: 1 zlatá mince.");	};		
	if (spell == SPL_Firebolt)				{	PLAYER_TALENT_RUNES[SPL_Firebolt] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Firebolt, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy OHNIVÝ ŠÍP: 1 síra.");	};		
	if (spell == SPL_Icebolt)				{	PLAYER_TALENT_RUNES[SPL_Icebolt] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Icebolt, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy LEDOVÝ ŠÍP: 1 ledový kâemen.");};		
	if (spell == SPL_LightHeal)				{	PLAYER_TALENT_RUNES[SPL_LightHeal] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_LightHeal, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy LÉČBA LEHKÝCH ZRANĘNÍ: 1 léčivá rostlina.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy VYVOLÁNÍ GOBLINÍHO KOSTLIVCE: 1 kost z goblina.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy OHNIVÁ KOULE: 1 pryskyâice.");	};	
	if (spell == SPL_Zap)					{	PLAYER_TALENT_RUNES[SPL_Zap] 					= TRUE;	CreateInvItems (ScrollTrader, ItSc_Zap, 1);						
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy MALÝ BLESK: 1 kamenný krystal.");	};		
	if (spell == SPL_SummonWolf)			{	PLAYER_TALENT_RUNES[SPL_SummonWolf]				= TRUE;	CreateInvItems (ScrollTrader, ItSc_SumWolf, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy VYVOLÁNÍ VLKA: 1 vlčí kůže.");	};		
	if (spell == SPL_WINDFIST)				{	PLAYER_TALENT_RUNES[SPL_WINDFIST] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_WINDFIST, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy VĘTRNÁ PĘST: 1 uhlík.");	};		
	if (spell == SPL_Sleep)					{	PLAYER_TALENT_RUNES[SPL_Sleep] 					= TRUE;	CreateInvItems (ScrollTrader, ItSc_Sleep, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy SPÁNEK: 1 stéblo trávy. Lze použít i alchymistické náčiní.");	};		
	if (spell == SPL_MediumHeal)			{	PLAYER_TALENT_RUNES[SPL_MediumHeal] 			= TRUE;	CreateInvItems (ScrollTrader, ItSc_MediumHeal, 1);				
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy LÉČBA STÂEDNĘ TĘŽKÝCH ZRANĘNÍ: 1 léčivá bylina.");	};		
	...
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy BLESK: 1 kamenný krystal a 1 ledový kâemen.");	};	
	...
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy ARMÁDA TEMNOT: 1 kost z kostlivce, 1 černá perla, 1 srdce kamenného golema a 1 srdce démona.");	};		
	if (spell == SPL_Shrink)				{	PLAYER_TALENT_RUNES[SPL_Shrink] 				= TRUE;	CreateInvItems (ScrollTrader, ItSc_Shrink, 1);					
	B_LogEntry (TOPIC_TalentRunes,"Pâísady pro vytvoâení runy ZMENŠENÍ NESTVŮRY: 1 kost z goblina a 1 trolí tesák.");	};		
