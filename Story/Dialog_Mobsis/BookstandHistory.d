...
FUNC VOID Use_BookstandHistory1_S1()		//Geschichtsbücher
{
	...
	{	
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Takového protivníka si je lepší nękolika dobâe míâenými ranami držet od tęla a pak ho v pâekvapit náhlým výpadem."						);
...
FUNC VOID Use_BookstandHistory2_S1()		//Geschichtsbücher
{
	...
	{	
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Tento ostrov je proslulý zejména tím, že se na nęm nachází slavné Hornícké údolí. Jeho povęst je pochybná, neboă je po mnoho let obklopovala neprostupná magická bariéra, a proto sloužilo jako vęznice pro trestance z celé âíše."						);
					...
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLines	( nDocID,  1, "Půda za hranicemi Khorinisu je velmi úrodná, proto tu najdete mnoho selských usedlostí, kde se pęstují zejména tuâíny a chovají ovce."						); 
					Doc_PrintLine	( nDocID,  1, ""						);
					...
					Doc_PrintLine	( nDocID,  1, ""						);
					Doc_PrintLines	( nDocID,  1, "Uprostâed ostrova stojí starobylý Innosův klášter, který vedou mágové ohnę a kde provádęjí své magické a alchymistické výzkumy a také lisují víno."						);
...
FUNC VOID Use_BookstandHistory3_S1()		//Geschichtsbücher
{
	...
	{	
		...
		nDocID = 	Doc_Create		()			  ;							
					...
					Doc_PrintLines	( nDocID,  0, "Všude, kde žije více lidí pohromadę, musí dodržovat určitá pravidla pokojného soužití - jak v klášteâe, tak ve męstę či na statku."						);
					...
					Doc_PrintLines	( nDocID,  1, "Chce-li se vyhnout vęzení, musí zaplatit tučnou pokutu. "						);
