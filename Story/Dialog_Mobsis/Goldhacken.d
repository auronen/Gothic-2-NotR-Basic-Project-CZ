	...
	if (B_GoldMob_Bestimmung() == TRUE)
	{
		if (CurrentChance <= Hero_HackChance)
		{
			...
			&& (MultiNugget >= 8)
			{
				...
				PrintScreen ("Vykopal si 3 nugety zlata.", -1, -1, FONT_ScreenSmall, 2);	
				Truemmer_Count = 0;
			...
			&& (MultiNugget >= 5)
			{
				...
				PrintScreen ("Vykopal si 2 nugety zlata.", -1, -1, FONT_ScreenSmall, 2);	
				Truemmer_Count = 0;
			...
			else
			{
				...
				PrintScreen ("Vykopal si 1 nugety zlata.", -1, -1, FONT_ScreenSmall, 2);	
				Truemmer_Count = 0;
				GoldCounter = (GoldCounter +1);
			};
		}
		...
		{
			PrintScreen ("Nevykopal si žádný nuget.", -1, -1, FONT_ScreenSmall, 2);	
			Truemmer_Count = (Truemmer_Count +1);
		};
	}
	else
	{
		PrintScreen ("Už tu žádné zlato není.", -1, -1, FONT_ScreenSmall, 2);	
		B_ENDPRODUCTIONDIALOG ();
	};
};
//*******************************************************
...
FUNC VOID PC_Goldhacken_Addon_Chance_Info()
{
	...
	{
		ConcatText = ConcatStrings ("tęžký začátečník (", IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 40) 
	{
		ConcatText = ConcatStrings ("slušný kopáč (" , IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 55) 
	{
		ConcatText = ConcatStrings ("velice dobrý kopáč (", IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 75) 
	{
		ConcatText = ConcatStrings ("rozený kopáč ( ", IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 90) 
	{
		ConcatText = ConcatStrings ("krutę dobrý kopáč ( ", IntToString (Hero_HackChance));
	}
	else if (Hero_HackChance < 98) 
	{
		ConcatText = ConcatStrings ("Pán kopáč ( ", IntToString (Hero_HackChance));
	}
	else
	{
		ConcatText = ConcatStrings ("Zlatokopec-guru ( ", IntToString (Hero_HackChance));
	};
