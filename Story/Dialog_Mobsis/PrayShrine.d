...
FUNC VOID PC_PrayShrine_Pray_Info()
{
	if (ShrineIsObsessed == TRUE)
		...
		{		
			...
			Info_AddChoice (PC_PrayShrine_Pray,"Chci se pomodlit a nic nevęnovat.",PC_PrayShrine_Pray_NoPay);
			
			if (Npc_HasItems (hero,ItMi_Gold) >=10)
			{
				Info_AddChoice (PC_PrayShrine_Pray,"Chci se pomodlit a vęnovat 10 zlaăáků.",PC_PrayShrine_Pray_SmallPay);
			};
			if (Npc_HasItems (hero,ItMi_Gold) >=50)
			{
				Info_AddChoice (PC_PrayShrine_Pray,"Chci se pomodlit a vęnovat 50 zlaăáků.",PC_PrayShrine_Pray_MediumPay);
			};
			if (Npc_HasItems (hero,ItMi_Gold) >=100)
			{
				Info_AddChoice (PC_PrayShrine_Pray,"Chci se pomodlit a vęnovat 100 zlaăáků.",PC_PrayShrine_Pray_BigPay);
			};
		};
}; 
