...
{	
	...
	AI_Output (self, other, "DIA_PAL_9_JOIN_09_02"); //Jinak âečeno: Vybráni mohou být jen ti nejlepší. Ale TY do toho máš hodnę daleko, hochu!
};
...
INSTANCE DIA_PAL_9_LOCATION(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Proč jste vy, paladinové, pâišli do Khorinisu?";
};                       
...
{	
	AI_Output (other, self, "DIA_PAL_9_LOCATION_15_00"); //Proč jste vy, paladinové, pâišli do Khorinisu?
	AI_Output (self, other, "DIA_PAL_9_LOCATION_09_01"); //Do toho ti nic není, pâíteli! Musí ti stačit, že jsme tu v Innosových a králových službách!
};
...
{	
	...
	|| (other.guild == GIL_KDF)
	{
		if (Kapitel <= 4)
		{
			...
			{
				AI_Output (self, other, "DIA_PAL_9_STANDARD_09_01"); //Teë když víme, že máme co do činęní s draky, začne náš velitel určitę brzy nęco dęlat.
			}
			else
			{
				AI_Output (self, other, "DIA_PAL_9_STANDARD_09_02"); //Ještę stále nemáme žádné zprávy od našich mužů z Hornického údolí. To je velmi znepokojivé.
			};
		};

		if (Kapitel >= 5)
		{
			AI_Output (self, other, "DIA_PAL_9_STANDARD_09_03"); //Díky Innosovi! Dračí hrozba byla zažehnána. Teë už se musíme jen vypoâádat s tęmi skâety a cesta k rudę je volná.
		};
	}
	else //KEIN Paladin
	{
		AI_Output (self, other, "DIA_PAL_9_STANDARD_09_04"); //Jestli je to všechno, co mi můžeš âíct, tak už se ztraă, pâíteli!
	};
};
