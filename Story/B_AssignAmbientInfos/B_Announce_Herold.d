...
func void B_Announce_Herold ()
{
	...
	AI_Output (self ,self,"DIA_Herold_Announce_04_00"); //Slyšte, občané Khorinisu! Podle pâímého rozkazu ctihodného lorda Hagena vstupuje v platnost následující naâízení.
	...
	if (Kapitel <= 2)
	{
		...
		if (randy == 0)
		{
			AI_Output (self ,self,"DIA_Herold_Announce_04_01"); //UZ bezpečnostních důvodů se vstup do lesů a divočiny kolem męsta důraznę nedoporučuje.
		...
		else if (randy == 1)
		{
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_04"); //Všichni občané našeho męsta, kteâí dokážou držet zbraŕ, jsou naléhavę žádáni, aby posílili âady královské domobrany.
		}
		else if (randy == 2)
		{
		...
		else if (randy == 3)
		{
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_10"); //Každý obyvatel Khorinisu, který se proviní nezákonnou činností, se bude okamžitę zodpovídat lordu Andremu.
		}
		else
		{
	...
	{
			AI_Output (self ,self,"DIA_Herold_Announce_04_16"); //Zvęsti, které vypovídají o pâítomnosti draků v Hornickém údolí Khorinisu, se nezakládají na pravdę.
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_18"); //Abychom tyto smęšné zvęsti vyvrátili, byla do Hornického údolí vyslána jednotka statečných paladinů pro vedením jednoho z nich, jenž se v té oblasti dokonale vyzná.
	}
	else	//Kapitel 5
	{
		randy = Hlp_Random (2);
...
/* 
			...
			AI_Output (self ,self,"DIA_Herold_Announce_04_09"); //Každý obyvatel Khorinisu, který se proviní nezákonnou činností, se bude okamžitę zodpovídat lordu Andremu.
