...
func void DIA_Valentino_HALLO_Info ()
{
	...
	|| (hero.guild == GIL_PAL)
	{
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_01"); //Jmenuji se Valentino. Snažím se, abych tento den, který mi Innos dal, neznesvętil žádnou prací.
		AI_Output			(other, self, "DIA_Valentino_HALLO_15_02"); //Dávej pozor na to, co âíkáš.
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_03"); //Ach, omluv mę. Nechtęl jsem Tvou Milost nijak urazit, ó, prosím za odpuštęní, pane.
	}
	else
	{
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_04"); //Copak se neumíš chovat? To je nechutné!
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_05"); //Poâád tahle špinavá chátra! Nemáš být náhodou nękde v práci?
		AI_Output			(self, other, "DIA_Valentino_HALLO_03_06"); //Ale počítám, že takového ošuntęlého chlápka jako ty si každý všimne, co?
		...
		AI_Output			(other, self, "DIA_Valentino_HALLO_15_08"); //Vidím, že s tebou si užiju poâádnou srandu.
	};
};
...
instance DIA_Valentino_WhoAgain		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Ptal jsem se tę, kdo jsi!";
};
...
{
	AI_Output (other, self, "DIA_Valentino_Add_15_01"); //Ptal jsem se tę, kdo jsi!
	...
	AI_Output			(self, other, "DIA_Valentino_HALLO_wasmachstdu_03_02"); //Trápení neznám, však moudrostí a zlatem oplývám a ženy mi padají k nohám, sotva mne spatâí.
	AI_Output			(self, other, "DIA_Valentino_HALLO_wasmachstdu_03_03"); //Jestli máš problémy, nech si je pro sebe, já je nepotâebuju.
};
...
instance DIA_Valentino_Manieren		(C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Zdá se mi, že bych ti męl uštędâit menší lekci slušného chování!";
};
...
{
	AI_Output (other, self, "DIA_Valentino_Add_15_02"); //Zdá se mi, že bych ti męl uštędâit menší lekci slušného chování!
	//AI_Output			(other, self, "DIA_Valentino_HALLO_klappe_15_00"); //Für deine dünnen Ärmchen hast du 'ne ganz schön große Klappe!
	AI_Output			(self, other, "DIA_Valentino_HALLO_klappe_03_01"); //Mnę to nevadí. Jen si mne klidnę zbij. Až se zítra ráno probudím, zase mi bude dobâe.
...
instance DIA_Valentino_WASNUETZLICHES		(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Chceš mi âíci ještę nęco jiného?";
};
...
{
	AI_Output (other, self, "DIA_Valentino_Add_15_03"); //(klidnę) Chceš mi âíci ještę nęco jiného?
	...
	&& (other.guild != GIL_KDF)
	{
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_01"); //Získej si vęhlas, nikdy nikomu nic neslibuj, ber si, co chceš, a dávej si pozor, aby ses nedostal do sporu s domobranou.
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_02"); //Nebo se žárlivými manželi, samozâejmę - ti jsou ze všech nejhorší, to ti povím.
		Valentino_Lo_Perm = TRUE;
	}
	else if (Valentino_Hi_Perm == FALSE)
	&& ( (other.guild == GIL_PAL) || (other.guild == GIL_KDF) )
	{
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_03"); //Pro muže tvého postavení není nic nemožné. Prostę jen musíš poznat, kdy je čas k činu.
		AI_Output (self, other, "DIA_Valentino_WASNUETZLICHES_03_04"); //Tak neztrácej čas a jednej!
		Valentino_Hi_Perm = TRUE;
	}
	else
	{
		AI_Output (self ,other,"DIA_Valentino_WASNUETZLICHES_03_05"); //Už jsem ti âekl všechno, co potâebuješ vędęt - zbytek je na tobę.
	};
};
