...
{	
	...
	Info_AddChoice (DIA_MiltenOW_Hello,"Męl bych je znát?",DIA_MiltenOW_Hello_NO);
};
	// ------------------------------------
...
FUNC VOID DIA_MiltenOW_Hello_YES()
{
	...
	Info_ClearChoices (DIA_MiltenOW_Hello);
	Info_AddChoice (DIA_MiltenOW_Hello,"Samozâejmę, že si kluky pamatuju.",DIA_MiltenOW_Hello_Friends);
...
{	
	...
	B_LogEntry (TOPIC_RescueGorn,"Velitel Garond nechal Gorna zavâít. Snad tak trochu naletęl, a tak bychom mu męli pomoci z nesnází.");
};
// ************************************************************
...
INSTANCE DIA_MiltenOW_Equipment(C_INFO)
{
	...
	permanent	= FALSE;
	description = "Mohl bys mi dát nęjakou výbavu?";
};                       
...
INSTANCE DIA_MiltenOW_Versteck (C_INFO)
{
	...
	permanent	= FALSE;
	IMPORTANT 	= FALSE;
	description = "Mám odpovęë od Gorna... "; 
}; 
...
func void DIA_MiltenOW_Mana_BACK()
{
	...
	{
		AI_Output (self, other, "DIA_MiltenOW_Mana_03_00"); //Tvoje magická síla je veliká. Pâíliš velká na to, abych ti ji mohl pomoci ještę zvýšit.
	};
	Info_ClearChoices (DIA_MiltenOW_Mana);
};
