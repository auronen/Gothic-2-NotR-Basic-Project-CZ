	...
	if (Got_HealObsession_Day<=(Wld_GetDay()-2))  
		{
			...
			if (Npc_HasItems (self,ItPo_HealObsession_MIS))
			{
				...
				AI_Output			(self, other, "DIA_Vatras_DI_OBSESSION_05_02"); //Nicménę vždycky pamatuj: moje prostâedky na zbavení tęch nočních můr jsou velmi omezené.
...
func void DIA_Vatras_DI_PEDROTOT_Info ()
{
	...
	&& (Npc_IsDead(Pedro_DI)) 
	{
		...
		AI_Output			(self, other, "DIA_Vatras_DI_PEDROTOT_05_04"); //Vím o tvé odporné dohodę s hady. Jsem hluboce otâesen tím, jak jsem nakonec pochybil.
		DIA_Vatras_DI_PEDROTOT_VatrasSucked = TRUE;
		AI_StopProcessInfos (self);
	}
	else
	{
		AI_Output			(self, other, "DIA_Vatras_DI_PEDROTOT_05_05"); //Tak ho pâiveë sem na palubu. Pâedáme ho úâadům v Khorinisu.
...
INSTANCE DIA_Vatras_DI_Talente (C_INFO)
{
	...
	description		= "Nauč mę nęco ze svých schopností."; 
};
...
{
	AI_Output			(self, other, "DIA_Vatras_DI_Talente_Circle_5_05_00"); //Stal jsi se mágem pátého Kruhu. Používej své vędomosti s rozvahou.
	Info_ClearChoices (DIA_Vatras_DI_Talente);
...
func void DIA_Vatras_DI_UndeadDragonDead_Info ()
{
	...
	if (DIA_Vatras_DI_UndeadDragonDead_OneTime == FALSE)
	{
		...
		{
			AI_Output (other, self, "DIA_Vatras_DI_UndeadDragonDead_15_03"); //Můžu teë odejít, nebo máte vy mágové v zásobę ještę nęjaký kostlivce, co potâebujou opustit tenhle svęt?
		}
		else
		{
