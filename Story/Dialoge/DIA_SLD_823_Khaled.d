...
{
	AI_Output (other, self, "DIA_Khaled_Hallo_15_00"); //Jsi v poâádku?
	if (Npc_KnowsInfo (other, DIA_Lee_WannaJoin))
	{
		AI_Output (self, other, "DIA_Khaled_Hallo_11_01"); //Tak co, chceš se k nám tedy pâidat? Máš aspoŕ nęjakou zbraŕ?
		Khaled_weiter = TRUE;
		Log_CreateTopic (Topic_SoldierTrader,LOG_NOTE);
		B_LogEntry (Topic_SoldierTrader,"Khaled obchoduje se zbranęmi.");
	}
	else
	{
...
instance DIA_Khaled_TRADE		(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Jaké zbranę mi můžeš nabídnout?";
	trade		= TRUE;
};
...
{
	AI_Output (other, self, "DIA_Khaled_TRADE_15_00"); //Jaké zbranę mi můžeš nabídnout?
	B_GiveTradeInv (self);
...
instance DIA_Khaled_WannaJoin		(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Nevadilo by ti, kdybych se k vám pâidal?";
};
...
{
	AI_Output (other, self, "DIA_Khaled_WannaJoin_15_00"); //Nevadilo by ti, kdybych se k vám pâidal?
	AI_Output (self, other, "DIA_Khaled_WannaJoin_11_01"); //Jestli jsi prošel zkouškou, klidnę se za tebe pâimluvím.
...
instance DIA_Khaled_Woher		(C_INFO)
{
	...
	permanent	= FALSE;
	description	= "Jak jsi dostal k žoldákům?";
};
...
{
	AI_Output (other, self, "DIA_Khaled_Woher_15_00"); //Jak jsi dostal k žoldákům?
	AI_Output (self, other, "DIA_Khaled_Woher_11_01"); //Pâišel jsem z jihu spolu se Sylviem. Patâili jsme k žoldnéâské armádę, která bojovala proti skâetům.
};
...
func void DIA_Khaled_AboutSylvio_Info ()
{
	...
	AI_Output (self, other, "DIA_Khaled_AboutSylvio_11_01"); //Je to nebezpečný chlápek! A spousta žoldáků, co pâišla s ním, ho poâád poslouchá. Radši by ses s ním nemęl pouštęt do kâížku.
};
...
func void DIA_Khaled_AboutLee_Info ()
{
	...
	AI_Output (self, other, "DIA_Khaled_AboutLee_11_01"); //Myslím, že jedinę Lee se opravdu stará, abychom se dostali z tohohle ostrova.
	AI_Output (self, other, "DIA_Khaled_AboutLee_11_02"); //Chce se vydat na pevninu, ale nejspíš si tu chce ještę s nękým nęco vyâídit.
	AI_Output (self, other, "DIA_Khaled_AboutLee_11_03"); //V každém pâípadę dęlá svou práci daleko líp, než by to kdy dokázal Sylvio.
	AI_Output (self, other, "DIA_Khaled_AboutLee_11_04"); //Krom toho je to dobrý vůdce. Vętšinou si tu můžeme dęlat, co chceme.
};
