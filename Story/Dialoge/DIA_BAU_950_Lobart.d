...
func void DIA_Lobart_STOLENCLOTHS_DontHaveIt ()
{
	...
	AI_Output (self, other, "DIA_Lobart_STOLENCLOTHS_DontHaveIt_05_01"); //Prodals je, co? (âve) Já ti ukážu, mladíku!
...
func void DIA_Lobart_Hallo_ForTheKing()
{
	...
	AI_Output (self, other, "DIA_Lobart_Hallo_ForTheKing_05_03"); //Já ti âeknu kde: ve męstę, za širokými hradbami.
...
func void DIA_Lobart_BuyClothes_Info ()
{
	...
	Info_AddChoice		(DIA_Lobart_BuyClothes, "Tak mi tedy dej ten pracovní odęv (ochrana: zbranę 15, šípy 15).", DIA_Lobart_BuyClothes_BUY );
};
	...
	if 	(hero.guild == GIL_NONE)
	 {
		...
		AI_Output (self, other, "DIA_Lobart_AufstandInfo_05_03"); //Ale od té doby, co jsou v Khorinisu paladinové, nás vojáci z domobrany navštęvují čím dál tím častęjc a už si pomalu berou úplnę všechno.
...
instance DIA_Lobart_OnarStory (C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "Âekni mi nęco o tom Onarovi.";
};
...
instance DIA_Lobart_WhichSide (C_INFO)
{
	...
	permanent 	= FALSE;
	description	= "A co ty? Na čí stranę jsi? Rolníci, nebo král?";
};
...
func void DIA_Lobart_WorkNOW_Info ()
{
	...
	if (hero.guild == GIL_NONE)
	{
		...
		|| (Lobart_Kleidung_Verkauft == TRUE)
		{
			AI_Output (self, other, "DIA_Lobart_WorkNOW_05_05"); //Ale když na tebe tak koukám, musím âíct: vezmi si ty šaty.
		}
		else
		{
	...
	Info_AddChoice		(DIA_Lobart_WorkNOW, "Já mám tahat ze zemę tuâín? To si musíš dęlat srandu!", DIA_Lobart_WorkNOW_WannaFoolMe );
...
func void DIA_Lobart_RuebenRunning_Info ()
{
	...
	//	Log_AddEntry (TOPIC_BauerLobart,"Ich habe Lobart seine Rüben gebracht, er hat mich dafür entlohnt."); 
		...
		if (hero.guild == GIL_NONE)
			{
				Info_AddChoice		(DIA_Lobart_RuebenRunning, "Udęlej mi za ty hadry lepší cenu!",	DIA_Lobart_RuebenRunning_Billiger );
			};
...
{
	AI_Output (other, self, "DIA_Lobart_RuebenRunning_Billiger_15_00"); //Udęlej mi za ty hadry lepší cenu!
...
instance DIA_Lobart_ANDREHELPLOBART		(C_INFO)
{
	...
	description	 = 	"Andre mę poslal za tebou. Máš pro mę nęjakou práci?";
};
...
func void DIA_Lobart_DMT_Info ()
{
	...
	{
		AI_Output			(self, other, "DIA_Lobart_DMT_05_04"); //Jenom zase nęjaká choroba. Už moc dlouho jen leží v posteli. Stává se jí to často.
	};
...
instance DIA_Lobart_ORKPROBLEM		(C_INFO)
{
	...
	description	 = 	"Máš problém se skâety?";
};
...
{
	AI_Output			(other, self, "DIA_Lobart_ORKPROBLEM_15_00"); //Máš problém se skâety?
