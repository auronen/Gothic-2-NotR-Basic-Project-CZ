...
{	
	AI_Output (other,self ,"DIA_Gorn_DI_Hallo_15_00"); //Všechno v poâádku?

	if (ORkSturmDI == FALSE)
		{
			AI_Output (self ,other,"DIA_Gorn_DI_Hallo_12_01"); //Zatím ano. Nezapomeŕ, že bych chtęl vidęt nęjakou poâádnou akci.
		}
	else
		{
			AI_Output (self ,other,"DIA_Gorn_DI_Hallo_12_02"); //Je tam nahoâe víc skâetů? Zaveë je ke mnę. Právę se začínám dostávat do nálady.
		};
};
...
{	
	AI_Output (other, self , 	"DIA_Gorn_DI_Teach_15_00"); //Potâebuji výcvik.
...
FUNC VOID DIA_Gorn_DI_Teach_2H_5 ()
{
	...
	{
		AI_Output(self,other,"DIA_Gorn_DI_Teach_2H_5_12_00"); //Drž zbraŕ o nęco výš. Tenhle kryt by prorazil i slepec svou holí.
	};
	Info_ClearChoices 	(DIA_Gorn_DI_Teach);
	Info_AddChoice 		(DIA_Gorn_DI_Teach,	DIALOG_BACK		,DIA_Gorn_DI_Teach_Back);
	Info_AddChoice		(DIA_Gorn_DI_Teach, B_BuildLearnString(PRINT_Learn2h5	, B_GetLearnCostTalent(other, NPC_TALENT_2H, 5))			,DIA_Gorn_DI_Teach_2H_5);
...
func void DIA_Gorn_DI_UndeadDragonDead_Info ()
{
	...
	AI_Output			(self ,other, "DIA_Gorn_DI_UndeadDragonDead_12_01"); //Samozâejmę. Jen si pâedstav, co by se stalo, kdyby tady už nebyla.

	if (DIA_Gorn_DI_UndeadDragonDead_OneTime == FALSE)
		{
			if (hero.guild == GIL_DJG)
			{
				AI_Output			(self ,other, "DIA_Gorn_DI_UndeadDragonDead_12_02"); //Co bude teë?
				AI_Output			(other ,self, "DIA_Gorn_DI_UndeadDragonDead_15_03"); //Nic nebude. Chci domů.
				AI_Output			(self ,other, "DIA_Gorn_DI_UndeadDragonDead_12_04"); //(smęje se) Domů? A kde to má být? Pokud vím, nic takového tę nečeká.
				AI_Output			(self ,other, "DIA_Gorn_DI_UndeadDragonDead_12_05"); //Nech mę hádat. Zapadneme do další hospody a ztâískáme se do nęmoty.
	...
	if (Npc_KnowsInfo(other, DIA_Biff_DI_plunder))
			{
				AI_Output			(self ,other, "DIA_Gorn_DI_UndeadDragonDead_12_08"); //Nękdo to musí vyâídit Biffovi, jinak na tomhle ostrovę shnije.
	...
	AI_Output			(self ,other, "DIA_Gorn_DI_UndeadDragonDead_12_10"); //Už tady nemáme co pohledávat. Âekni kapitánovi, aby se pâipravil k vyplutí.
	AI_StopProcessInfos (self);
