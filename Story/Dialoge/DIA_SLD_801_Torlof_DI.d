...
FUNC VOID DIA_Torlof_DI_Hallo_Info()
{
	...
	if (ORkSturmDI == FALSE)
	{
		AI_Output (self ,other,"DIA_Torlof_DI_Hallo_01_01"); //Zatím. Ale to se může bęhem okamžiku zmęnit. Dávej si pozor.
	}
	else
	{
		AI_Output (self ,other,"DIA_Torlof_DI_Hallo_01_02"); //Pokud ti mizerní skâeti zůstanou tam, kde jsou, tak netuším, v čem by mohl být problém.
...
func void DIA_Torlof_DI_UndeadDragonDead_Info ()
{
	...
	AI_Output			(self ,other, "DIA_Torlof_DI_UndeadDragonDead_01_01"); //Nic jiného jsem ani nečekal. Jak to vypadá? Můžeme konečnę odplout?

	Info_ClearChoices	(DIA_Torlof_DI_UndeadDragonDead);
	Info_AddChoice	(DIA_Torlof_DI_UndeadDragonDead, "Dej mi ještę pár minut.", DIA_Torlof_DI_UndeadDragonDead_moment );
...
{
	AI_Output			(other, self, "DIA_Torlof_DI_UndeadDragonDead_moment_15_00"); //Dej mi ještę pár minut.
	AI_Output			(self, other, "DIA_Torlof_DI_UndeadDragonDead_moment_01_01"); //Pospęš si!
	AI_StopProcessInfos (self);
};
