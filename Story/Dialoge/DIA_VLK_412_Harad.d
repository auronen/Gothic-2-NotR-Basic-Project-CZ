...
func void DIA_Harad_Taugenichts_Info ()
{
	...
	AI_Output (self, other, "DIA_Harad_Taugenichts_12_05"); //Jestli se ti povede nęjakého skolit, pak tę pâijmu do učení.
...
instance DIA_Harad_OrcRunning (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Promluvme si ještę o té skâetí vęci...";
};
...
{
	AI_Output (other, self, "DIA_Harad_OrcRunning_15_00"); //Promluvme si ještę o té skâetí vęci...
	AI_Output (self, other, "DIA_Harad_OrcRunning_12_01"); //(stroze) Co?
	
	Info_ClearChoices (DIA_Harad_OrcRunning);	
	Info_AddChoice (DIA_Harad_OrcRunning, "Tvůj skâet už je teë mrtvý!", DIA_Harad_OrcRunning_Done);
...
func void DIA_Harad_LEHRLING_Info ()
{
	...
	|| ( (MIS_HakonBandits == LOG_SUCCESS) && (Harad_HakonMission == TRUE) )
	{
		...
		{
			AI_Output (self, other,"DIA_Harad_LEHRLING_12_08"); //Thorben prý s tebou ještę nikdy nemluvil.
		};
		...
		{
			AI_Output (self, other,"DIA_Harad_LEHRLING_12_18"); //Constantino o tobę ještę nikdy neslyšel.
		};
		...
		if (Matteo.aivar[AIV_TalkedToPlayer] == TRUE)
		{
			...
			{
				AI_Output (self, other,"DIA_Harad_LEHRLING_12_20"); //A Matteo hlásí, že mu ještę poâád nęco dlužíš. Nevím, co mezi sebou máte, ale męl bys to dát rychle do poâádku.
			}
			else	
			{	
...
func void B_HaradSmithChoices()
{
	...
	{
		Info_AddChoice	(DIA_Harad_TeachSmith, B_BuildLearnString("Nauč mę kování"	   , B_GetLearnCostTalent(other, NPC_TALENT_SMITH, WEAPON_Common))			,DIA_Harad_TeachSmith_Common);
	};
};
// ------------------------------------------------------------
...
func void DIA_Harad_Goldsmith_Info ()
{
	...
	AI_Output (self ,other,"DIA_Harad_Goldsmith_12_01"); //Nęjaký dobrý kováâ se prý pâidal k žoldákům na Onarovę farmę.
