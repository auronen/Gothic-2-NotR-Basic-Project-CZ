...
{	
	...
	AI_Output (self ,other,"DIA_Lutero_Hallo_13_02"); //No, hlavnę vzácné a neobvyklé vęci. Vždycky jsem se snažil vyhovęt zákazníkovu pâání.
	
	Log_CreateTopic (TOPIC_CityTrader, LOG_NOTE);
	B_LogEntry (TOPIC_CityTrader, "Lutero prodává v horní čtvrti neobvyklé a vzácné vęci."); 
};
//************************************************************
//	GetLost
//************************************************************
...
{	
	if (other.guild == GIL_NONE)
	{
		AI_Output (self ,other,"DIA_Lutero_GetLost_13_00"); //Zmizni, koâene! Nemáš co dęlat? Tak si koukej sehnat práci - ale nękde jinde!
	}
	else
	{
		AI_Output (self ,other,"DIA_Lutero_GetLost_13_01"); //Co tady chceš, novici? Nemęl bys být v klášteâe?
	};
	AI_StopProcessInfos (self);
};
//************************************************************
...
INSTANCE DIA_Lutero_Snapper(C_INFO)
{
	...
	permanent	= FALSE;
	description = "Hledáš nęco konkrétního?";
};                       
...
{	
	AI_Output (other ,self,"DIA_Lutero_Snapper_15_00"); //Hledáš nęco konkrétního?
	AI_Output (self ,other,"DIA_Lutero_Snapper_13_01"); //Pro svůj současný úkol potâebuju drápy chŕapavce.
	AI_Output (self ,other,"DIA_Lutero_Snapper_13_02"); //Ale ne jen tak ledajaké drápy. Musí to být nęco zvláštního - drápy mocné bestie, co už zabila mnoho mužů, nebo nęco takového.
	AI_Output (other ,self,"DIA_Lutero_Snapper_15_03"); //Kde najdu chŕapavce?
	AI_Output (self ,other,"DIA_Lutero_Snapper_13_04"); //Na tomhle ostrovę jich pár je, ale vętšina z nich žije v Hornickém údolí.
	AI_Output (other ,self,"DIA_Lutero_Snapper_15_05"); //A co z toho budu mít?
	
	if (other.guild == GIL_KDF)
	{
		AI_Output (self ,other,"DIA_Lutero_Hello_13_06"); //Můžu ti opatâit nęjaký runový kámen.
	}
	else
	{
		AI_Output (self ,other,"DIA_Lutero_Hello_13_07"); //Můžu ti dát prsten nepâemožitelnosti.
	};
	AI_Output (other ,self,"DIA_Lutero_Hello_15_08"); //Uvidím, co se dá dęlat.
	
	Log_CreateTopic (Topic_Lutero,LOG_MISSION);
	Log_SetTopicStatus (Topic_Lutero,LOG_RUNNING);
	B_LogEntry (Topic_Lutero,"Obchodník Lutero hledá drápy zvlášă silného chŕapavce.");
...
INSTANCE DIA_Lutero_Kralle(C_INFO)
{
	...
	permanent	= FALSE;
	description = "Mám pro tebe nęjaké zajímavé drápy chŕapavce.";
};                       
...
{	
	AI_Output (other ,self,"DIA_Lutero_Kralle_15_00"); //Mám pro tebe nęjaké zajímavé drápy chŕapavce.
	...
	if (other.guild == GIL_KDF)
	{
		...
		AI_Output (self ,other,"DIA_Lutero_Hello_13_03"); //Až půjdeš z męsta do hospody, projdeš pod mostem.
		AI_Output (self ,other,"DIA_Lutero_Hello_13_04"); //Je tam jeskynę, moji pâátelé tam mají v truhle uložené runové kameny. Tady je klíč.
