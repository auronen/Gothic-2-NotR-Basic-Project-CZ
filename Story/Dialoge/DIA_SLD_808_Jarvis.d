...
{	
	...
	AI_Output (self, other, "DIA_Jarvis_DieLage_04_01"); //Právę teë máme skutečný problém. Mezi námi žoldáky se vytváâí dvę frakce.
...
{	
	...
	AI_Output (self, other, "DIA_Jarvis_TwoFronts_04_05"); //On souhlasil s tím, že Lee zůstane velitelem, ale nyní se snaží poštvat ostatní žoldáky proti nęmu a jeho zámęrům.
	AI_Output (self, other, "DIA_Jarvis_TwoFronts_04_06"); //Hodnę Leeových lidí si nemyslí, že je to velkej problém. Ale já znám týpky jako je Sylvio.
	AI_Output (self, other, "DIA_Jarvis_TwoFronts_04_07"); //Aby docílil svého, bude to klidnę hnát na ostâí nože.
};
...
instance DIA_Jarvis_LeesPlan (C_INFO)
{
	...
	permanent	= FALSE;
	description = "Víš, co chce Lee dęlat?";
};                       
...
{	
	...
	AI_Output (self, other, "DIA_Jarvis_SylviosPlan_04_02"); //Âíká, že zbytek paladinů by si nás tady neodvážil napadnout, a chce z toho vytęžit co nejvíc.
...
{	
	...
	AI_Output (self, other, "DIA_Jarvis_MissionKO_04_01"); //Jednoduché. Zmlaă pár Sylviových hochů! Tímhle způsobem budou obę strany pâesnę vędęt, kde stojíš.
...
instance DIA_Jarvis_SylviosMen (C_INFO)
{
	...
	permanent	= FALSE;
	description = "Kteâí ze žoldáků jsou Sylviovi muži?";
};                       
...
{	
	...
	B_LogEntry (TOPIC_JarvisSLDKo,"K Sylviovým chlapům patâí kromę Sylvia ještę jeho velitel Bullco, Rod, Sentenza, Fester a Raoul.");
};
...
instance DIA_Jarvis_HowManyLeft (C_INFO)
{
	...
	permanent	= TRUE;
	description = "Kolik Sylviových mužů mám ještę na seznamu?";
};                       
...
{	
	...
	|| (Sentenza.aivar[AIV_KilledByPlayer] == TRUE)
	{
		...
		{
			AI_Output (other, self, "DIA_Jarvis_HowManyLeft_15_04"); //Sentenza se pokusil ze mne dostat nęjakou zlatku - to nebylo chytrý.
		}
		else
		{
...
{	
	...
	if (Kapitel >= 4)
	{
		...
		{
			AI_Output (self, other, "DIA_Jarvis_PERM_04_04"); //Sylvio je konečnę pryč. Po tom, co slyšel o dracích, on a jeho pár chlapů smęâovali do tęžaâské kolonie.
			AI_Output (self, other, "DIA_Jarvis_PERM_04_05"); //Pravdępodobnę si myslel, že by tam mohlo být nęco víc.
			Jarvis_SylvioComment = TRUE;
		}
		else
		{
			AI_Output (self, other, "DIA_Jarvis_PERM_04_06"); //Nic, teë už je všechno v klidu. Nemohu se dočkat toho, až uvidím, jak tohle všechno dopadne.
		};
	};
};
