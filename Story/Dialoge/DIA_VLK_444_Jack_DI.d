...
func void DIA_Jack_DI_GREET_Info ()
{
	if (ORkSturmDI == FALSE)
	{
		AI_Output			(self ,other, "DIA_Jack_DI_GREET_14_00"); //Doufám, že víš, co dęláš, chlapče.
		AI_Output			(other ,self, "DIA_Jack_DI_GREET_15_01"); //Už to nebude trvat dlouho.
		AI_Output			(self ,other, "DIA_Jack_DI_GREET_14_02"); //Zachovej si chladnou hlavu a vraă se v jednom kuse.
		AI_StopProcessInfos (self);
	}
	else
	{
		...
		{
			AI_Output			(other ,self, "DIA_Jack_DI_GREET_15_03"); //Co tam dole dęláš?
			B_GivePlayerXP (XP_Ambient);
			DIA_Jack_DI_GREET_OneTime = TRUE;
		};

		AI_Output			(self ,other, "DIA_Jack_DI_GREET_14_04"); //I kdyby se męlo samo peklo zjevit, já tu loë odsud dostanu. S tebou, nebo bez tebe. Slyšíš?
...
instance DIA_Jack_DI_UndeadDragonDead		(C_INFO)
{
	...
	description = 	"Dokázal jsem to. Zničil jsem nepâítele.";
...
{
	AI_Output			(other ,self, "DIA_Jack_DI_UndeadDragonDead_15_00"); //Dokázal jsem to. Zničil jsem nepâítele.
	AI_Output			(self ,other, "DIA_Jack_DI_UndeadDragonDead_14_01"); //Doufal jsem, že všechno dobâe dopadne. Ale musim pâiznat, že posledních pár hodin jsem si tím nebyl vůbec jistej.
	...
	Info_AddChoice	(DIA_Jack_DI_UndeadDragonDead, "Ještę počkej.", DIA_Jack_DI_UndeadDragonDead_moment );
...
{
	AI_Output			(other, self, "DIA_Jack_DI_UndeadDragonDead_moment_15_00"); //Ještę počkej. Je tu nęco dalšího, co musím udęlat.
	AI_Output			(self, other, "DIA_Jack_DI_UndeadDragonDead_moment_14_01"); //Hoë sebou, jasný? Nechci tu dostat kurdęje.
	AI_StopProcessInfos (self);
};
