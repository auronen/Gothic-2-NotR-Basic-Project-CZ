...
func void DIA_Dar_Hallo_Ja()
{
	...
	AI_Output (self, other, "DIA_Dar_Hallo_Ja_03_03"); //(kâení se) Mám své zdroje.
	Info_ClearChoices (DIA_Dar_Hallo);	
};
...
FUNC VOID DIA_Dar_PERM_Info()
{
	...
	&& (Dar_einmal == FALSE)
	{
		AI_Output (self, other, "DIA_Dar_PERM_03_01"); //(sarkasticky) Nękdy se nechám zmlátit od nękolika pomstychtivých feăáků kvůli týhle trávę...
		Dar_einmal = TRUE;
	}
		else //DEFAULT
	{
...
INSTANCE DIA_Dar_DuDieb (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Cipher mi âekl, že mu nękdo ukradnul jeho balíček trávy.";
};
...
FUNC VOID DIA_Dar_WoPaket_Info()
{
	...
	{	
		...
		AI_Output (other, self, "DIA_Dar_WoPaket_15_03"); //Vypadá to, že potâebuješ další výprask?
		...
		AI_Output (self, other, "DIA_Dar_WoPaket_03_05"); //Bylo to v pâístavu blízko stavitelů lodí. To je všechno, na co si vzpomínám.
	
		Dar_Dieb = TRUE;
		B_LogEntry (Topic_CipherPaket,"Dar se pâiznal, že balík s trávou ukradl on. Prodal jej v khorinisské pâístavní čtvrti, hned vedle loëaâovy dílny.");
	}
	else
	{
...
{
	AI_Output (other, self, "DIA_Dar_Pilztabak_15_00"); //Už jsi nękdy balil tabák?
...
func void DIA_Dar_ORCRING_Info ()
{
	...
	AI_Output			(self, other, "DIA_Dar_ORCRING_03_01"); //Jistę. To je pravda. Sylvio je na míle daleko a vzal půlku posádky s sebou.
	AI_Output			(self, other, "DIA_Dar_ORCRING_03_02"); //To je jedno. Mám alespoŕ lepší šanci stát se jedním z Leeových chlapců. Všechno, co potâebuju, je se porvat.
...
func void DIA_Dar_ORCRING_necken ()
{
	...
	if ((hero.guild == GIL_MIL) || (hero.guild == GIL_PAL))
	{
		AI_Output			(self, other, "DIA_Dar_ORCRING_necken_03_05"); //Když o tom tak pâemýšlím, rozbití tvojí lebky je pâesnę to, čím bych si mohl u Leeovým chlapců vysloužit ten správný respekt.

		Info_ClearChoices	(DIA_Dar_ORCRING);
		Info_AddChoice	(DIA_Dar_ORCRING, "INemám čas na tyhle kecy.", DIA_Dar_ORCRING_necken_no );
...
func void DIA_Dar_ORCRING_necken_schlagen ()
{
	...
	AI_Output			(self, other, "DIA_Dar_ORCRING_necken_schlagen_03_01"); //Ooch, nemůžu čekat.
	AI_StopProcessInfos (self);
...
func void DIA_Dar_ORCRING_wie ()
{
	...
	AI_Output			(self, other, "DIA_Dar_ORCRING_wie_03_02"); //Tâeba odznak od vůdce skâetů nebo nęco takového. Prapor, prsten, nebo tak nęco.
	AI_Output			(self, other, "DIA_Dar_ORCRING_wie_03_03"); //S ničím menším tady prorazit nejde. To je zâejmé.
...
func void DIA_Dar_FIGHTAGAINSTPALOVER_Info ()
{
	...
	AI_Output			(self, other, "DIA_Dar_FIGHTAGAINSTPALOVER_03_01"); //Nechci si tu nikoho znepâátelit, tak na ten nesmysl prostę zapomeŕme, jo?
	B_GivePlayerXP (XP_Ambient);
	AI_StopProcessInfos (self);
};
...
func void DIA_Dar_BRINGORCELITERING_geld_no ()
{
	...
	AI_Output			(self, other, "DIA_Dar_BRINGORCELITERING_geld_no_03_01"); //A já zase myslím, že je to nęjak moc. Bez urážky, tahle záležitost se mi zdá nęjak podezâelá.
	Info_ClearChoices	(DIA_Dar_BRINGORCELITERING);
};
...
func void DIA_Dar_BRINGORCELITERING_was ()
{
	...
	AI_Output			(self, other, "DIA_Dar_BRINGORCELITERING_was_03_01"); //Buë si vezmi peníze, nebo tenhle amulet, který jsem... no, âeknęme že jsem jej pâed časem nęjak získal.
