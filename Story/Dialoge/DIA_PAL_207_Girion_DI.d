...
{
	AI_Output			(other, self, "DIA_Girion_DI_Hallo_15_00"); //Je všechno v poâádku?

	if (ORkSturmDI == TRUE)
		{
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_01"); //Skâeti jsou opravdu stateční, že se vydali proti mnę.
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_02"); //Jeden by âekl, že mají dost rozumu na to, aby se postavili paladinovi z královské stráže.
		}
	else if (hero.guild == GIL_PAL)
		{
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_03"); //Také bych âekl. Zabere ti tvůj úkol tady na útesu hodnę času?
		}
	else
		{
			AI_Output			(self, other, "DIA_Girion_DI_Hallo_08_04"); //Nechovej se ke mnę jako k idiotovi. Udęlej co musíš a pâedej mi velení lodi.
		};
};
...
INSTANCE DIA_Girion_DI_Teach(C_INFO)
{
	...
	permanent	= TRUE;
	description = "Uč mę, pak se odsud dostaneme rychleji.";
};                       
...
{	
	AI_Output (other,self 	,"DIA_Girion_DI_Teach_15_00"); //Uč mę, pak se odsud dostaneme rychleji.
	AI_Output (self ,other 	,"DIA_Girion_DI_Teach_08_01"); //Zdá se, že nemám na výbęr.
...
func void DIA_Girion_DI_Teach_CROSSBOW_1()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_CROSSBOW_1_08_00"); //Nikdy moc nezvedej lokty. Budeš pak mít tuhou mušku a zákonitę zkroutíš zbraŕ.
	};
...
func void DIA_Girion_DI_Teach_CROSSBOW_5()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_CROSSBOW_5_08_00"); //Pâed výstâelem si vždycky uvolni levou ruku. Bude se ti pak lépe míâit.
	};
...
FUNC VOID DIA_Girion_DI_Teach_1H_1 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_1H_1_08_00"); //Ostâí musí letęt vętrem jako rákos povętâím.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
FUNC VOID DIA_Girion_DI_Teach_1H_5 ()
{
	...
	{
			AI_Output (self ,other,"DIA_Girion_DI_Teach_1H_5_08_00"); //Svou sílu používej s rozmyslem. Když se bezhlavę rozbęhneš proti nepâíteli, moc úspęchu nesklidíš.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1	, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
FUNC VOID DIA_Girion_DI_Teach_2H_1 ()
{
	...
	{
		AI_Output(self,other,"DIA_DIA_Girion_DI_Teach_2H_1_08_00"); //Nemęj pâíliš pevné sevâení, abys mohl pâesnę trefit.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1	, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
FUNC VOID DIA_Girion_DI_Teach_2H_5 ()
{
	...
	{
		AI_Output(self,other,"DIA_Girion_DI_Teach_2H_5_08_00"); //Nezapomeŕ se bránit. Nejlepší obranou je vyhýbat se protivníkovým ranám.
	};
	Info_ClearChoices 	(DIA_Girion_DI_Teach);
	Info_AddChoice 		(DIA_Girion_DI_Teach,	DIALOG_BACK		,DIA_Girion_DI_Teach_Back);
	Info_AddChoice		(DIA_Girion_DI_Teach, B_BuildLearnString(PRINT_LearnCrossBow1	, 		  B_GetLearnCostTalent(other, NPC_TALENT_CROSSBOW, 1))		,DIA_Girion_DI_Teach_CROSSBOW_1);
...
instance DIA_Girion_DI_OrcEliteRing		(C_INFO)
{
	...
	description	 = 	"Mám prsten skâetího vůdce.";
};
...
{
	AI_Output			(other, self, "DIA_Girion_DI_OrcEliteRing_15_00"); //Mám prsten skâetího vůdce.

	if (DIA_Girion_DI_OrcEliteRing_OneTime == FALSE)
	{
		AI_Output			(self, other, "DIA_Girion_DI_OrcEliteRing_08_01"); //Myslím, že by s tím mohl lord Hagen nęco udęlat.
...
func void DIA_Girion_DI_OrcEliteRing_geben ()
{
	...
	AI_Output			(self, other, "DIA_Girion_DI_OrcEliteRing_geben_08_01"); //Díky. Stejnę si nemyslím, že by ti mohl k nęčemu být.
	Info_ClearChoices	(DIA_Girion_DI_OrcEliteRing);
...
func void DIA_Girion_DI_OrcEliteRing_behalten ()
{
	...
	AI_Output			(self, other, "DIA_Girion_DI_OrcEliteRing_behalten_08_01"); //Jak myslíš. Stejnę ti k ničemu nebude.
	Info_ClearChoices	(DIA_Girion_DI_OrcEliteRing);
};
...
instance DIA_Girion_DI_UndeadDragonDead		(C_INFO)
{
	...
	description = 	"Teë můžeme konečnę vyrazit.";
};
...
{
	AI_Output			(other ,self, "DIA_Girion_DI_UndeadDragonDead_15_00"); //Teë můžeme konečnę vyrazit.
	...
	if (DIA_Girion_DI_UndeadDragonDead_OneTime == FALSE)
		{
			AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_02"); //Teë mi okamžitę tu loë pâedej.
			AI_Output			(other ,self, "DIA_Girion_DI_UndeadDragonDead_15_03"); //Ještę nejsme v Khorinisu. A upâímnę âečeno, zpátky do Khorinisu se mi moc nechce.
			AI_Output			(other ,self, "DIA_Girion_DI_UndeadDragonDead_15_04"); //Už jsem tam byl dost dlouho.
			AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_05"); //Myslel jsem si to. Ale teë není ani správný čas, ani správné místo, abychom si nęco takového vyjasŕovali. Probereme to, až budeme zpátky na lodi.
			AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_06"); //No a co. Stejnę jsem rád, že tę mám zpátky na palubę v jednom kuse.
			
			if (hero.guild == GIL_PAL)
			{
				AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_07"); //Jsi skutečný Innosův válečník. Už o tom nemám žádných pochyb. Jsem rád, že tę máme na své stranę.
			}
			else
			{
				AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_08"); //I když jsi lump, zachoval ses nakonec správnę.
			};
			
			DIA_Girion_DI_UndeadDragonDead_OneTime = TRUE;
		};

	AI_Output			(self ,other, "DIA_Girion_DI_UndeadDragonDead_08_09"); //Dobrá. Teë jdi za kapitánem, aă nás odsud vyvede.
	AI_StopProcessInfos (self);
};
