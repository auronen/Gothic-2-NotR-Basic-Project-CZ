...
func void DIA_SylvioDJG_HelloAgain_Info ()
{
	...
	{
		AI_Output			(self, other,"DIA_SylvioDJG_HelloAgain_09_00"); //Dobrá, aă se propadnu! Koukáš, jak si vydęlat, co? Vędęl jsem to! Máš žoldáckou duši.
	...
	{
		AI_Output			(self, other, "DIA_SylvioDJG_HelloAgain_09_02"); //Hej, ty! Mágové mę znervózŕují! Bęž nękam jinam! Tady pro tebe nic není.
	}
	else
	{
		AI_Output			(self, other, "DIA_SylvioDJG_HelloAgain_09_03"); //Hej, ty, paladine! Vraă se zpátky do tęch svejch dolů. Tady pro tebe nic není.
	};
	
	AI_Output			(other, self, "DIA_SylvioDJG_HelloAgain_15_04"); //Jasný. Nikdo se nechce dęlit o svou koâist.
	AI_Output			(self, other, "DIA_SylvioDJG_HelloAgain_09_05"); //Pâesnę tak. Zmizni.
};
...
func void DIA_Sylvio_VERSAGER_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_VERSAGER_09_01"); //Nehraj si na tvrëáka, pískle, jinak skončíš jako ty mizerný svinę, co se válej támhle na snęhu.
};
...
func void DIA_Sylvio_DEINELEUTE_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_DEINELEUTE_09_01"); //Ne tak docela. Žádný velký ztráty. Ti idioti stejnę nestáli za nic.
};
...
func void DIA_Sylvio_WASISTPASSIERT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_09_01"); //Nebyli schopný se dostat tam pâes ty ledový obry. Zůstali tam.
	...
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "K čemu ti bude dobrý, když to udęlám?", DIA_Sylvio_WASISTPASSIERT_washastdudavon );
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Co z toho budu mít?", DIA_Sylvio_WASISTPASSIERT_warum );
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Proč to nevyâídíš sám?", DIA_Sylvio_WASISTPASSIERT_selbst );

	Log_CreateTopic (TOPIC_SylvioKillIceGolem, LOG_MISSION);
	Log_SetTopicStatus(TOPIC_SylvioKillIceGolem, LOG_RUNNING);
	B_LogEntry (TOPIC_SylvioKillIceGolem,"Sylvio se bojí dvou ledových golemů pâed vchodem do zamrzlé části Hornického údolí."); 

	MIS_DJG_Sylvio_KillIceGolem = LOG_RUNNING;
};
func void DIA_Sylvio_WASISTPASSIERT_selbst ()
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_selbst_15_00"); //Proč to nevyâídíš sám?
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_selbst_09_01"); //Hele, nech si ty nafoukaný âeči.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_selbst_09_02"); //Jak to já vidím, seš podęlanej strachy.
	
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Nemám v úmyslu pâistoupit na tvoji hru.", DIA_Sylvio_WASISTPASSIERT_keinInteresse );
...
func void DIA_Sylvio_WASISTPASSIERT_warum ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_warum_09_01"); //Hodnę. Âeknęme... 1000 zlatých. To zní dobâe, ne?
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_washastdudavon_15_00"); //K čemu ti bude dobrý, když to udęlám?
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_washastdudavon_09_01"); //K tomu samýmu, co tobę. Ale abych ti odpovędęl na otázku, chci se dostat do té zamrzlé oblasti.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_washastdudavon_09_02"); //Bohužel, v cestę mi stojej ty zatracený vęci.
	
	Info_AddChoice	(DIA_Sylvio_WASISTPASSIERT, "Co chceš v té zamrzlé oblasti dęlat?", DIA_Sylvio_WASISTPASSIERT_Eisregion );

};

func void DIA_Sylvio_WASISTPASSIERT_keinInteresse ()
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_NEIN_15_00"); //Nemám v úmyslu pâistoupit na tvoji hru.
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_15_00"); //Co chceš v té zamrzlé oblasti dęlat?
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_09_01"); //Ty s tęma otázkama nepâestaneš, co? Dobrá, povím ti to.
	AI_Output			(self, other, "DIA_Sylvio_WASISTPASSIERT_keinInteresse_09_02"); //Tady, za tím srázem, je taková ledová pustina, jakou jsi nikdy pâedtím nevidęl.
...
instance DIA_Sylvio_ICEGOLEMSKILLED		(C_INFO)
{
	...
	description	 = 	"Cesta je volná! Ti ledoví obâi už tam zaclánęt nebudou.";
};
...
{
	AI_Output			(other, self, "DIA_Sylvio_ICEGOLEMSKILLED_15_00"); //Cesta je volná! Ti ledoví obâi už tam zaclánęt nebudou.
...
func void DIA_Sylvio_WASJETZT_Info ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_09_01"); //Fajn. Âekl bych, že to vypadá pęknę na levačku.
	...
	Info_AddChoice	(DIA_Sylvio_WASJETZT, "Nebudu za tebe dęlat špinavou práci.", DIA_Sylvio_WASJETZT_nein );
	if (DJG_Sylvio_PromisedMoney  == TRUE)
	{
	Info_AddChoice	(DIA_Sylvio_WASJETZT, "Nejdâív bych rád vidęl ty prachy.", DIA_Sylvio_WASJETZT_Geld );
	};
};
func void DIA_Sylvio_WASJETZT_trennen ()
{
	AI_Output			(other, self, "DIA_Sylvio_WASJETZT_trennen_15_00"); //Bojíš se?
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_trennen_09_01"); //Nesmysl, pâestaŕ žvanit. Jdeme.
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASJETZT_nein_15_00"); //Nebudu za tebe dęlat špinavou práci.
...
{
	AI_Output			(other, self, "DIA_Sylvio_WASJETZT_Geld_15_00"); //Nejdâív bych rád vidęl ty prachy.
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_Geld_09_01"); //Až dostaneme toho draka, budeš mít penęz, kolik uneseš.
...
func void DIA_Sylvio_WASJETZT_jetztGeld ()
{
	...
	AI_Output			(self, other, "DIA_Sylvio_WASJETZT_jetztGeld_09_01"); //Buë půjdeš vpâedu, nebo si to s tebou vyâídim.
};
...
instance DIA_Sylvio_KOMMSTDU		(C_INFO)
{
	...
	description	 = 	"Myslel jsem, že máme namíâeno do té zamrzlé oblasti.";
};
...
{
	AI_Output			(other, self, "DIA_Sylvio_KOMMSTDU_15_00"); //Myslel jsem, že máme namíâeno do té zamrzlé oblasti.
...
instance DIA_Sylvio_DUHIER		(C_INFO)
{
	...
	description	 = 	"Potâebovat bych nęjakou pomoc.";
};
...
{
	AI_Output			(other, self, "DIA_Sylvio_DUHIER_15_00"); //Potâebovat bych nęjakou pomoc.
	AI_Output			(self, other, "DIA_Sylvio_DUHIER_09_01"); //Nesmysl. Vedeš si dobâe.
...
{
	AI_Output	(self, other, "DIA_SylvioDJG_WHATNEXT_09_00"); //Ledový drak je mrtvý! A teë mi dej všechno, co u sebe máš!
	...
	AI_Output	(self, other, "DIA_SylvioDJG_WHATNEXT_09_03"); //Tvoje nepodstatná role je právę u konce!
...
func void DIA_SylvioDJG_WHATNEXT_ATTACK ()
{
	...
	B_LogEntry (TOPIC_Dragonhunter,"Ten hajzl Sylvio si dęlal nároky na moje vítęzství nad ledovým drakem. Trochu jsme se kvůli tomu poškorpili."); 
...
INSTANCE DIA_SylvioDJG_IceWait1 (C_INFO)
{
	...
	important	= 0;	
	permanent	= 0;
	description	= "Ich bin soweit! Laß uns losgehen!";
};                       
