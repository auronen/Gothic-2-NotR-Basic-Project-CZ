...
{
	AI_Output (self ,other,"DIA_Hokurn_Hello_01_00"); //Nemáš nęco k pití?
	AI_Output (other,self ,"DIA_Hokurn_Hello_15_01"); //A asi to nemá být voda, co?
	AI_Output (self ,other,"DIA_Hokurn_Hello_01_02"); //To teda ne, ksakru!! Potâebuju nęjakej alkohol, abych se konečnę zbavil toho zatracenýho bolehlavu.
	AI_Output (self ,other,"DIA_Hokurn_Hello_01_03"); //Jestli se brzo nęčeho nenapiju, nejspíš mi pukne hlava.

	Info_ClearChoices (DIA_Hokurn_Hello);
	Info_AddChoice (DIA_Hokurn_Hello,"Pokusím se nęco obstarat.",DIA_Hokurn_Hello_No);
...
FUNC VOID DIA_Hokurn_Hello_No ()
{
	...
	Info_AddChoice (DIA_Hokurn_Hello,"Ještę jedna vęc ...",DIA_Hokurn_Hello_ASK1);
};

FUNC VOID DIA_Hokurn_Hello_ASK1 ()
{
	AI_Output (other,self ,"DIA_Hokurn_Hello_ASK1_15_00"); //Ještę jedna vęc...
	AI_Output (self ,other,"DIA_Hokurn_Hello_ASK1_01_01"); //(povzdech) Tys mi nerozumęl? VYPADNI!!!
...
FUNC VOID DIA_Hokurn_Hello_ASK2 ()
{
	...
	AI_Output (self ,other,"DIA_Hokurn_Hello_ASK2_01_01"); //(povzdech) Âekl sis o to.
...
{
	AI_Output (self ,other,"B_Hokurn_Sauf_01_00"); //(âíhnutí) Ha, tohle jsem potâeboval.
	AI_Output (self ,other,"B_Hokurn_Sauf_01_01"); //Teë můžu znova myslet s čistou hlavou. Co pro tebe můžu udęlat?
};
...
INSTANCE DIA_Hokurn_Drink   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Tady máš nęco k pití.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_Drink_15_00"); //Tady máš nęco k pití.
	AI_Output (self ,other,"DIA_Hokurn_Drink_01_01"); //(dychtivę) Dej to sem!!
...
{
	AI_Output (other,self ,"DIA_Hokurn_Question_15_00"); //Potâebuju nęjaké informace.
	AI_Output (self ,other,"DIA_Hokurn_Question_01_01"); //(otrávenę) Męl jsem dojem, že jsme si rozumęli. Mluvím jen s pâáteli.
	AI_Output (self ,other,"DIA_Hokurn_Question_01_02"); //A pâátelé ti nabídnou nęco k pití. Jasný? Teë odpal!
};
...
INSTANCE DIA_Hokurn_Learn   (C_INFO)
{
	...
	permanent   = FALSE;
	description = "Hledám nękoho, kdo by mę mohl nęčemu naučit.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_Learn_15_00"); //Hledám nękoho, kdo by mę mohl nęčemu naučit.
	AI_Output (self ,other,"DIA_Hokurn_Learn_01_01"); //Můžu ti ukázat pár vęcí. Jsem nejlepší bojovník na míle daleko.
	AI_Output (self ,other,"DIA_Hokurn_Learn_01_02"); //Jelikož jsme pâátelé, udęlám ti zvláštní cenu. 300 zlatých.

	Info_ClearChoices (DIA_Hokurn_Learn);
	Info_AddChoice	(DIA_Hokurn_Learn,"To je pro mę moc drahé.",DIA_Hokurn_Learn_TooExpensive);
...
{
	AI_Output (other,self ,"DIA_Hokurn_Learn_TooExpensive_15_00"); //To je pro mę moc drahé.
	AI_Output (self ,other,"DIA_Hokurn_Learn_TooExpensive_01_01"); //Moc drahé? To je nejmíŕ, co by si na mém místę âekl kdokoli jiný.
	AI_Output (self ,other,"DIA_Hokurn_Learn_TooExpensive_01_02"); //Pâemýšlej o tom.
...
INSTANCE DIA_Hokurn_PayTeacher   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Tady jsou peníze. Chci, abys mę učil.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_PayTeacher_15_00"); //Tady jsou peníze. Chci, abys mę učil.
...
INSTANCE DIA_Hokurn_DrinkAndLearn   (C_INFO)
{
	...
	permanent   = TRUE;
	description = "Tady je pro tebe nęco k pití.";
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_DrinkAndLearn_15_00"); //Tady je pro tebe nęco k pití.
	...
	AI_Output (self ,other,"DIA_Hokurn_DrinkAndLearn_01_01"); //Cítím se o moc líp. Jsem pâipraven na vše.
...
{	
	AI_Output (other,self ,"DIA_Hokurn_Teach_15_00"); //Začnęme s tréninkem.
	
	if (HokurnLastDrink < Wld_GetDay ())
	{
		AI_Output (self ,other,"DIA_Hokurn_Teach_01_01"); //Nejdâív mi pâines nęco k pití!
	}
	else
	{	 
		if (hero.guild == GIL_PAL)
		{
			AI_Output (self ,other,"DIA_Hokurn_Teach_01_02"); //Pak se uvidí, co můžeme dostat z tęch tvejch zatuhlejch paladinskejch kostí, jasný?
		}
		else if (hero.guild == GIL_KDF)
		{
			AI_Output (self ,other,"DIA_Hokurn_Teach_01_03"); //Co ty víš. Dokonce i mágové používají chladné zbranę.
		};
...
{
	AI_Output(self,other,"B_Hokurn_TeachedEnough_01_00"); //Na tuhle disciplínu už nepotâebuješ žádnýho učitele.
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_StayHere_15_00"); //A co tady vlastnę dęláš?
	AI_Output (self ,other,"DIA_Hokurn_StayHere_01_01"); //Nemám tušení, proč jsme sem pâišli, nebo kdy to začne.
...
FUNC VOID DIA_Hokurn_WhereDragon_Info()
{
	...
	AI_Output (self ,other,"DIA_Hokurn_WhereDragon_01_01"); //Co z toho budu mít, když ti to âeknu?
	...
	if Npc_HasItems (other,ITFO_BOOZE) >=1
	{
		Info_AddChoice (DIA_Hokurn_WhereDragon,"Ještę tu mám láhev ginu!",DIA_Hokurn_WhereDragon_Booze);	 
	};
};
...
FUNC VOID DIA_Hokurn_WhereDragon_FindMyself ()
{
	...
	AI_Output (self ,other,"DIA_Hokurn_WhereDragon_FindMyself_01_01"); //Męl by sis dávat pozor. Čeká tę spousta bojů.
	Info_ClearChoices (DIA_Hokurn_WhereDragon);
};
...
FUNC VOID DIA_Hokurn_WhereDragon_TooMuch ()
{
	...
	AI_Output 	(self ,other,"DIA_Hokurn_WhereDragon_TooMuch_01_01"); //Tak to na to zapomeŕ.
	Info_ClearChoices	(DIA_Hokurn_WhereDragon); 
};
...
{
	AI_Output (other,self ,"DIA_Hokurn_WhereDragon_Booze_15_00"); //Ještę tu mám láhev ginu!
	AI_Output (self ,other,"DIA_Hokurn_WhereDragon_Booze_01_01"); //Za poâádnou pitku bych zabil všechny draky svęta.
...
FUNC VOID DIA_Hokurn_Dragon_Info()
{
	...
	AI_Output (self ,other,"DIA_Hokurn_Dragon_01_01"); //Pâirozenę ti to neâeknu úplnę pâesnę, ale slyšel jsem, že by tu męli být celkem čtyâi.
	AI_Output (self ,other,"DIA_Hokurn_Dragon_01_02"); //V noci jsme zahlédli nad nejvyšší horou rudou záâi.
	AI_Output (self ,other,"DIA_Hokurn_Dragon_01_03"); //Pâísahám na hrob mé matky, pokud hledáš draka, tak když nikde jinde, tam ho určitę najdeš.
};
...
FUNC VOID DIA_Hokurn_AllDragonsDead_Info()
{
	...
	AI_Output 	(self ,other,"DIA_Hokurn_AllDragonsDead_01_01"); //Udęlej si výlet a vyprávęj své pohádky nękomu jinému.
};
