...
{	
	...
	if (ORkSturmDI == FALSE)
	{
		AI_Output	(self ,other,"DIA_Bennet_DI_Hello_06_01");	//Kovárna na téhle lodi je tak kapku zarezlá. Může za to ta moâská sůl. Vyrobit tam nęco komplikovanęjší může být docela problém. Ale zase na druhou stranu...
	}
	else
	{
		AI_Output	(self ,other,"DIA_Bennet_DI_Hello_06_02");	//Můžeme zaâídit, aby se skâeti nevrátili zpęt.
	};
};
...
instance DIA_Bennet_DI_TRADE		(C_INFO)
{
	...
	description	 = 	"Jaké zbranę mi můžeš prodat?";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_TRADE_15_00"); //Jaké zbranę mi můžeš prodat?
	B_GiveTradeInv (self);
	AI_Output			(self, other, "DIA_Bennet_DI_TRADE_06_01"); //Pouze ty nejlepší. To pâece víš.
};
...
instance DIA_Bennet_DI_Smith		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 = 	"Můžeš mę naučit své umęní?";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_Smith_15_00"); //Můžeš mę naučit své umęní?
	AI_Output			(self, other, "DIA_Bennet_DI_Smith_06_01"); //Záleží na tom, co chceš dęlat.
	...
	{
		Info_AddChoice		(DIA_Bennet_DI_Smith, B_BuildLearnString("Naučit se kováâství "	   , B_GetLearnCostTalent(other, NPC_TALENT_SMITH, WEAPON_Common))			,DIA_Bennet_DI_Smith_Common);
	};
...
{	
	AI_Output (other,self ,"DIA_Bennet_TeachSTR_15_00"); //Chtęl bych se stát silnęjším.
...
func void DIA_Bennet_DI_DragonEgg_Info ()
{
	...
	AI_Output			(self, other, "DIA_Bennet_DI_DragonEgg_06_03"); //Vím, na co myslíš. Zapomeŕ na to, nech si tu vęc. Já to nechci.
...
instance DIA_Bennet_DI_UndeadDragonDead		(C_INFO)
{
	...
	description	 = 	"Udęlali jsme všechno, co jsme potâebovali udęlat.";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DI_UndeadDragonDead_15_00"); //Udęlali jsme všechno, co jsme potâebovali udęlat.
	AI_Output			(self, other, "DIA_Bennet_DI_UndeadDragonDead_06_01"); //To rád slyším. Už jsem męl po krk téhle staré kovárny. Potâebuju se zase postavit na zem pevnýma nohama.
	AI_StopProcessInfos (self);
};
