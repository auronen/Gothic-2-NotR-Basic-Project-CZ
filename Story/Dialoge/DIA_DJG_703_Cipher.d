...
{
	AI_Output			(other, self, "DIA_Cipher_HALLO_15_00"); //Pękný tábor.
	AI_Output			(self, other, "DIA_Cipher_HALLO_07_01"); //To âíkáš ty. Všude jsou jen samý zvíâecí mrtvoly a poražený kmeny.
};
...
{
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_15_00"); //Co tady dęláš?
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_01"); //Nęjakou dobu tu jen posedávám a čekám na vhodný okamžik.
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_15_02"); //Vhodný okamžik? Na co?
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_03"); //Tamhle nahoâe by męl hnízdit drak. Od tý doby, co pâišel, tam vzniknul pęknę smradlavej močál.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_04"); //Moc dobâe se pamatuju doby, kdy tady kolem bylo úplnę sucho.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_05"); //Ale teë už tam nikdy nepůjdu sám.

	if ((Npc_IsDead(DJG_Rod)) == FALSE)
		{
			AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_06"); //Fajn. Rod je ještę poâád tady. Ale vezme nohy na ramena hned, jak uvidí první žravou štęnici.
		};

	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_07_07"); //A co ty? Ty jsi sem taky nepâišel jen tak pro zábavu, co? Tak tam půjdem spolu.

	B_LogEntry (TOPIC_Dragonhunter,"Podle Ciphera se prý v bažinách v Hornickém údolí usadil drak."); 

	Info_AddChoice	(DIA_CipherDJG_HELLOAGAIN, "Radęji zůstávám sám.", 	DIA_CipherDJG_HELLOAGAIN_GoAlone);
	Info_AddChoice	(DIA_CipherDJG_HELLOAGAIN, "Proč ne - můžu potâebovat nęjakou pomoc.", DIA_CipherDJG_HELLOAGAIN_GoTogether);
...
{
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_GoAlone_15_00"); //Radęji zůstávám sám.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_GoAlone_07_01"); //Jak je libo. Asi jsem se v tobę dost spletl.
	AI_StopProcessInfos (self);
};

func void DIA_CipherDJG_HELLOAGAIN_GoTogether ()
{
	AI_Output			(other, self, "DIA_CipherDJG_HELLOAGAIN_GoTogether_15_00"); //Proč ne - můžu potâebovat nęjakou pomoc.
	AI_Output			(self, other, "DIA_CipherDJG_HELLOAGAIN_GoTogether_07_01"); //Tak fajn. Konečnę odsud můžu vypadnout. Ten zápach je k nevydržení. Stačí âíct!
...
func void DIA_CipherDJG_GOTOGETHERAGAIN_Info ()
{
	...
	AI_Output			(self, other, "DIA_CipherDJG_GOTOGETHERAGAIN_07_01"); //Stačí âíct!
	DJG_SwampParty = TRUE;
};
...
instance DIA_CipherDJG_GO		(C_INFO)
{
	...
	description	 = 	"Teë by mohla být ta správná chvíle, abysme vyrazili!";
};
...
{
	AI_Output			(other, self, "DIA_CipherDJG_GO_15_00"); //Teë by mohla být ta správná chvíle, abysme vyrazili!
...
{
	AI_Output (self, other,"DIA_CipherDJG_SwampWait2_07_00"); //Ta bažina začíná hned za první zatáčkou. Asi bychom męli jít pâímo tudy.
...
func VOID DIA_CipherDJG_GoForSwampDragon_Info()
{
	...
	{
	AI_Output			(self, other, "DIA_CipherDJG_GoForSwampDragon_07_00"); //(kâičí) Útok!
	AI_StopProcessInfos	(self);
...
instance DIA_CipherDJG_WHATNEXT		(C_INFO)
{
	...
	description	 = 	"Močálový drak je po smrti! Co budeš dęlat teë?";
};
...
{
	AI_Output	(other, self, "DIA_CipherDJG_WHATNEXT_15_00"); //Močálový drak je po smrti! Co budeš dęlat teë?
	AI_Output	(self, other, "DIA_CipherDJG_WHATNEXT_07_01"); //Nemám tušení. Ještę jsem o tom nepâemýšlel, vážnę. Ale v každym pâípadę se teë můžeš vrátit do Khorinisu jako hrdina.
	AI_Output	(self, other, "DIA_CipherDJG_WHATNEXT_07_02"); //Vsadím se, že z toho jde docela dost vytęžit. Pâemýšlej o tom.

	B_LogEntry (TOPIC_Dragonhunter,"Když je močálový drak mrtvý, chce z toho Cipher vytâískat nęjaké prachy a stát se velkým 'hrdinou'. No, uvidíme."); 
