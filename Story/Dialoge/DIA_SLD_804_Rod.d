...
{	
	...
	{
		AI_Output (self, other, "DIA_Rod_Hello_06_01"); //(pohrdavę) Co ode mne chceš, blbečku?
...
func void DIA_Rod_Wette_Yes()
{
	...
	if (B_GiveInvItems (other,self, itmi_gold, 30))
	{
		...
		AI_Output (self, other, "DIA_Rod_Wette_Yes_06_03"); //(škodolibę) V poâádku, podíváme se tedy jak si silnej...
		...
		if (other.attribute[ATR_STRENGTH] >= 30)
		{
			...
			AI_Output (self, other, "DIA_Rod_Wette_Yes_06_06");//Tohle sem vůbec nečekal. Opravdu nevypadáš na to, že máš v sobę tolik síly.
...
func void DIA_Rod_Wette_GiveBack()
{
	...
	Info_ClearChoices (DIA_Rod_Wette);
	Info_AddChoice (DIA_Rod_Wette, "(Dát mu zbraŕ.)", DIA_Rod_Wette_GiveBack2);
};
...
func void DIA_Rod_Wette_KeepIt()
{
	...
	AI_Output (other, self, "DIA_Rod_Wette_KeepIt_15_02"); //Myslím, že si ho ponechám ještę o trochu déle.
...
{	
	...
	if (Kapitel <= 2)	
	{
		if (other.guild == GIL_NONE)
		{
			...
			{
				AI_Output (self, other, "DIA_Rod_PERM_06_02"); //Jsem zvędavý, jestli ti dovolí se k nám pâidat.
			};
		}
	...
	if (Kapitel >= 3)
	{
		AI_Output (self, other, "DIA_Rod_PERM_06_05"); //Jestli všechny ty povídačky o dracích jsou skutečnę pravdivé, męli bychom dát pár lidí dohromady a jít ty mrchy pozabíjet!
	};
