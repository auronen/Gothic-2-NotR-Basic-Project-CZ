...
func void DIA_Angar_DI_HALLO_Info ()
{
	...
	if ((Npc_IsDead(UndeadDragon)) == FALSE)
	{
		AI_Output			(self, other, "DIA_Angar_DI_HALLO_04_01"); //Od tý doby, co jsme na ostrovę, je ta bolest čím dál tím nesnesitelnęjší.
		AI_Output			(self, other, "DIA_Angar_DI_HALLO_04_02"); //Čert aby to spral. Už by to snad mohlo skončit.
	}
	else
	{
		AI_Output			(self, other, "DIA_Angar_DI_HALLO_04_03"); //Nedęlej si se mnou starosti. Prostę odsud vypadni.
	};
...
{
	AI_Output			(self, other, "DIA_Angar_DI_ORKS_04_00"); //Męli bysme sebou zatracenę mrsknout. Další skâetí útok nás bude stát život.
	AI_Output			(other, self, "DIA_Angar_DI_ORKS_15_01"); //Dęlám, co můžu.
...
{
	AI_Output			(other, self, "DIA_Angar_DI_ORKS_follow_15_00"); //Tak mi pomoz a pâestaŕ skučet.
...
func void DIA_Angar_DI_ORKS_no ()
{
	...
	AI_Output			(self, other, "DIA_Angar_DI_ORKS_no_04_01"); //To se ti snadno âekne. Nejspíš se tady z toho zblázním.
	AI_StopProcessInfos (self); 
};
...
{
	AI_Output			(other, self, "DIA_Angar_DI_FOLLOW_15_00"); //Zůstaŕ vzadu.

	if (Npc_GetDistToWP(self,"SKELETTE")<4000) 
		{
			AI_Output			(other, self, "DIA_Angar_DI_FOLLOW_15_01"); //O zbytek se postarám sám.
			AI_Output			(self ,other, "DIA_Angar_DI_FOLLOW_04_02"); //Hodnę štęstí.
		...
		{	
			AI_Output			(self, other, "DIA_Angar_DI_FOLLOW_04_03"); //Jasná vęc.
...
{
	AI_Output			(other, self, "DIA_Angar_DI_FOLLOWAGAIN_15_00"); //Pojë za mnou.
...
{
	AI_Output			(self, other, "DIA_Angar_DI_FOLLOWSTOP_04_00"); //Aarh! Ta bolest mę pâivádí k šílenství!
};
...
func void DIA_Angar_DI_UNDEADDRGDEAD_Info ()
{
	...
	AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_01"); //Můžeme už odsud konečnę vypadnout?
	AI_Output			(other, self, "DIA_Angar_DI_UNDEADDRGDEAD_15_02"); //Jo. Nepâítel byl poražen.
	AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_03"); //Tak to nesmíme dál ztrácet čas. Zajdi kapitánovi âíct, aby zvednul kotvy.

	if (SC_KnowsMadPsi == TRUE)
	{
		AI_Output			(other, self, "DIA_Angar_DI_UNDEADDRGDEAD_15_04"); //Doufám, že vás, odpadlých členů sekty, není víc.
		AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_05"); //Všechno je možné. I když Pátrači byli velmi důkladní. Dokonce málem dostali i mę. Kdo ví?
	};

	AI_Output			(other, self, "DIA_Angar_DI_UNDEADDRGDEAD_15_06"); //Co budeš dęlat dál?
	AI_Output			(self, other, "DIA_Angar_DI_UNDEADDRGDEAD_04_07"); //Možná se usadím na venkovę a začnu farmaâit. Válčení už mę dost unavuje.
	AI_StopProcessInfos (self);
	Npc_ExchangeRoutine	(self,"Start");
};
