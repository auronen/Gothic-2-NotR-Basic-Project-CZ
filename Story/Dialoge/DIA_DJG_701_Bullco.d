...
{
	AI_Output			(other, self, "DIA_BullcoDJG_HALLO_15_00"); //Co se dęje??
	AI_Output			(self, other, "DIA_BullcoDJG_HALLO_06_01"); //Je tady zatracená zima, chlape! Proč Sylvio trval na tom, abysme šli zrovna sem?
	AI_Output			(self, other, "DIA_BullcoDJG_HALLO_06_02"); //Existuje spousta jinejch míst, co stojej alespoŕ za nęco!
	AI_StopProcessInfos (self);
};
...
func void DIA_BullcoDJG_WARTEMAL_Info ()
{
	...
	if (Npc_KnowsInfo(other, DIA_Bullco_WASNUN))
		{
		AI_Output			(self, other, "DIA_BullcoDJG_WARTEMAL_06_01"); //Prostę mi pâestaŕ lézt na nervy.
		}
	else
		{
		AI_Output			(self, other, "DIA_BullcoDJG_WARTEMAL_06_02"); //Teë ne! Mrznu! Jediný, co chci, je odsud vypadnout, jak nejrychlejš to půjde.
		};
...
{
	AI_Output			(self, other, "DIA_Bullco_SYLVIODEAD_06_00"); //Čert aby to spral. Sylvio je mrtvej.
...
func void DIA_Bullco_WASNUN_Info ()
{
	...
	AI_Output			(self, other, "DIA_Bullco_WASNUN_06_01"); //Nemám tušení. Hádám, že půjdu dál s ostatníma drakobijcema. Pokud teda o mę ještę budou stát.

	Info_AddChoice	(DIA_Bullco_WASNUN, "Męl bych ti utrhnout hlavu.", DIA_Bullco_WASNUN_kopfab );
...
func void DIA_Bullco_WASNUN_woandere ()
{
	...
	AI_Output			(self, other, "DIA_Bullco_WASNUN_woandere_06_01"); //Myslím, že jo. Alespoŕ jsem jednoho z nich vidęl tam na âece.

	Info_AddChoice	(DIA_Bullco_WASNUN, "Vezmi mę k ostatním drakobijcům.", DIA_Bullco_WASNUN_woandere_zuihnen );
};
func void DIA_Bullco_WASNUN_woandere_zuihnen ()
{
	AI_Output			(other, self, "DIA_Bullco_WASNUN_woandere_zuihnen_15_00"); //Vezmi mę k ostatním drakobijcům.
	...
	Info_AddChoice	(DIA_Bullco_WASNUN, "Jak je libo. Jsem si jistej, že ho najdu.", DIA_Bullco_WASNUN_woandere_zuihnen_alleine );
	Info_AddChoice	(DIA_Bullco_WASNUN, "Považuj se za sluhu a já tę nechám naživu.", DIA_Bullco_WASNUN_woandere_zuihnen_lebenlassen );
...
{
	AI_Output			(other, self, "DIA_Bullco_WASNUN_woandere_zuihnen_lebenlassen_15_00"); //Považuj se za sluhu a já tę nechám naživu.
	AI_Output			(self, other, "DIA_Bullco_WASNUN_woandere_zuihnen_lebenlassen_06_01"); //Nech mę na pokoji.
	AI_StopProcessInfos (self);
};
	...
	if (B_GiveInvItems (other, self, ItMi_Gold,50))
		{
			...
			AI_Output			(self, other, "DIA_Bullco_WASNUN_woandere_zuihnen_Geld_06_02"); //Pojë za mnou. Vezmu tę na místo, kde jsem naposledy spatâil ostatní drakobijce.
			AI_StopProcessInfos (self);
			Npc_ExchangeRoutine	(self,"DJGVorposten");
		}
		else
		{
			AI_Output			(self, other, "DIA_Bullco_WASNUN_woandere_zuihnen_Geld_06_03"); //Nemáš 50 zlatých a já se do takovýho průseru prostę jen tak nepoženu. Ztraă se.
			AI_StopProcessInfos (self);
		};
};
...
func void DIA_Bullco_WASNUN_woandere_zuihnen_alleine ()
{
	...
	AI_Output			(self ,other, "DIA_Bullco_WASNUN_woandere_zuihnen_alleine_06_01"); //V tom pâípadę žádný vytrácení.
...
{
	AI_Output			(other, self, "DIA_Bullco_WASNUN_kopfab_15_00"); //Męl bych ti utrhnout hlavu.
	AI_Output			(self, other, "DIA_Bullco_WASNUN_kopfab_06_01"); //Sklapni už tu svojí velkou hubu.

	Info_AddChoice	(DIA_Bullco_WASNUN, "Do zbranę. Skončíme to hned.", DIA_Bullco_WASNUN_kopfab_angriff );


};
func void DIA_Bullco_WASNUN_kopfab_angriff ()
{
	AI_Output			(other, self, "DIA_Bullco_WASNUN_kopfab_angriff_15_00"); //Do zbranę. Skončíme to hned.
