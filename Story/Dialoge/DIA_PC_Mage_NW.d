...
{	
	...
	if (hero.guild == GIL_DJG)
	{
		...
		AI_Output (self ,other,"DIA_MiltenNW_KAP3_Hello_03_15"); //Mám samozâejmę radost, a vypadáš tak, že se tę musí každý skâet hned leknout.
...
{	
	AI_Output (other,self ,"DIA_MiltenNW_KAP3_Entry_15_00"); //Potâebuji se dostat do kláštera. Je to důležité!
	if (hero.guild == GIL_PAL)
	{
	...
	else
	{
		...
		AI_Output (self ,other,"DIA_MiltenNW_KAP3_Entry_03_03"); //Bez povolení mých nadâízených nesmím do kláštera nikoho vpouštęt.
...
instance DIA_MiltenNW_KnowWhereEnemy		(C_INFO)
{
	...
	PERMANENT 	 =  TRUE;
	description	 = 	"Vím, kde se nepâítel ukrývá. Je to malý ostrůvek nedaleko odsud.";
};
...
func void DIA_MiltenNW_WhereCaptain_Info ()
{
	...
	AI_Output			(self, other, "DIA_MiltenNW_WhereCaptain_03_01"); //Zeptej se Jorgena. Je to konec konců námoâník. Męl by být ještę poâád v klášteâe.
	...
	AI_Output			(self, other, "DIA_MiltenNW_WhereCaptain_03_03"); //Nejlepší asi bude promluvit si s Leem nebo jít do pâístavu v Khorinisu. Nic lepšího mę teë nenapadá.
...
func void DIA_MiltenNW_Mana_BACK()
{
	...
	{
		AI_Output (self, other, "DIA_MiltenNW_Mana_03_00"); //Tvoje magické síly jsou velké. Moc velké na to, abych ti je pomohl ještę zvýšit.
	};
	Info_ClearChoices (DIA_MiltenNW_Mana);
};
