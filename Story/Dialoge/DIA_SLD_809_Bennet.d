...
instance DIA_Bennet_BauOrSld (C_INFO)
{
	...
	permanent	= FALSE;
	description	= "Jsi s farmáâema, nebo žoldákama?";
};
...
{
	AI_Output (other, self, "DIA_Bennet_BauOrSld_15_00"); //Jsi s farmáâema, nebo žoldákama?
...
func void DIA_Bennet_WannaSmith_Pay()
{
	...
	if (B_GiveInvItems (other, self, itmi_gold, 30))
	{
		...
		B_LogEntry (Topic_SoldierTeacher,"Bennet naučí kováâskému umęní.");
	}
	else
	{
...
instance DIA_Bennet_WannaSmithORE (C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Nauč mne kovat zbranę z magické rudy!";
};
...
func void DIA_Bennet_WannaSmithORE_Info ()
{
	...
	if (PLAYER_TALENT_SMITH[WEAPON_Common] == FALSE)
	{
		...
		AI_Output (self, other, "DIA_Bennet_WannaSmithORE_06_02"); //Nejdâíve se musíš naučit ukovat obyčejný meč. Pak uvidíme.
	}
	else if (other.guild != GIL_SLD)
	&& 		(other.guild != GIL_DJG)
	{
		AI_Output (self, other, "DIA_Bennet_WannaSmithORE_06_03"); //Protože nejsi jedním z nás, tak bych v háji, kdybych ti vyzradil tajemství zbraní z magické rudy.
		...
		{	
			AI_Output (self, other, "DIA_Bennet_WannaSmithORE_06_06"); //Nic proti tobę. (kâení se) Vypadáš být v pohodę.
		};
	}
	else if (MIS_Bennet_BringOre != LOG_SUCCESS)
	{
		...
		AI_Output (self, other, "DIA_Bennet_WannaSmithORE_06_11"); //To jsem si myslel. Potâebuju nejménę 5 hroudek této rudy - nebo na to rovnou zapomeŕ.
...
instance DIA_Bennet_BringOre (C_INFO)
{
	...
	permanent	= FALSE;
	description	= "Tady máš tęch pęt kousků rudy.";
};
...
{
	AI_Output (other, self, "DIA_Bennet_BringOre_15_00"); //Tady máš tęch pęt kousků rudy.
...
instance DIA_Bennet_TeachSmith		(C_INFO)
{
	...
	permanent	= TRUE;
	description	= "Chci vędęt víc o kutí zbraní z magické rudy.";
};
...
func void DIA_Bennet_TeachSmith_Info ()
{
	...
	{
		AI_Output (self, other, "DIA_Bennet_TeachSmith_06_04"); //Poslyš. Myslím, že jsem právę dostal dokonalý nápad. Zbranę z magické rudy, potâísnęné dračí krví. A já pâesnę vím, jak na to půjdu!
		AI_Output (self, other, "DIA_Bennet_TeachSmith_06_05"); //(zubí se) Chceš to vędęt?
		Bennet_Kap5Smith = TRUE;
	}
	else
	{
		AI_Output (self, other, "DIA_Bennet_TeachSmith_06_06"); //Jaký druh zbranę bys chtęl vyrobit?
	};
...
func void DIA_Bennet_WhyPrison_Info ()
{
	...
	AI_Output 	(self ,other ,"DIA_Bennet_WhyPrison_06_04"); //Jak to mám vędęt? Męl bys mne odtud dostat.
...
func void DIA_Bennet_WhatHappened_Info ()
{
	...
	AI_Output 	(self ,other ,"DIA_Bennet_WhatHappened_06_06"); //Zbytek je už jednoduchej. Najednou u mne byla domobrana a ta mę uvrhla rovnou do týhle díry.
};
...
func void DIA_Bennet_Evidence_Info ()
{
	...
	AI_Output	(self , other,"DIA_Bennet_Evidence_06_01"); //Âíkají, že mají nęjakého svędka, který mne poznal.
...
func void DIA_Bennet_Present_Info ()
{
	...
	AI_Output 	(self ,other,"DIA_Bennet_Present_06_01"); //Slyšeli jsme o dracích, kteâí by męli prý bejt v údolí.
...
instance DIA_Bennet_DJG_ARMOR_M		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 =	"Stâednę tęžká drakobijecká zbroj. Ochrana: zbranę 120, šípy 120. (12000 zlaăáků)."; //Wenn ändern, dann bitte auch in der Info-Instanz. s.u.
};
...
{	
	...
	{
		AI_Output 	(self ,other,"DIA_Bennet_DJG_ARMOR_M_06_01"); //Velmi dobâe. Bude se ti to líbit.
...
instance DIA_Bennet_BetterArmor		(C_INFO)
{
	...
	permanent	 = 	FALSE;
	description	 =	"Vím, jak brnęní ještę víc vylepšit.";
};
...
{	
	...
	AI_Output 	(self ,other,"DIA_Bennet_BetterArmor_06_04"); //Mé nejnovęjší brnęní pâedčí všechno, co jsi doposud vidęl. Je teë lehké a masivní.
	AI_Output 	(self ,other,"DIA_Bennet_BetterArmor_06_05"); //Je PERFEKTNÍ.
	AI_Output 	(self ,other,"DIA_Bennet_BetterArmor_06_06"); //Můžeš ho koupit, jestli chceš. Nedávám tuhle nabídku jen tak nękomu a navíc cena zahrnuje pouze výrobní náklady.
};
...
instance DIA_Bennet_DJG_ARMOR_H		(C_INFO)
{
	...
	permanent	 = 	TRUE;
	description	 =	"Tęžká drakobijecká zbroj. Ochrana: zbranę 150, šípy 150. (20000 zlaăáků)."; //Wenn ändern, dann bitte auch in der Info-Instanz. s.u.
};
...
func void DIA_Bennet_RepairNecklace_Info ()
{
	...
	if (MIS_RescueBennet != LOG_SUCCESS)
	{
		...
		AI_Output (self ,other,"DIA_Bennet_RepairNecklace_06_03"); //Pâedtím ti tu vęc zâejmę neopravím.
	};	
...
func void DIA_Bennet_GiveInnosEye_Info ()
{
	...
	AI_Output			(self, other, "DIA_Bennet_GiveInnosEye_06_02"); //Můžeš potom ke mnę pâijít a dostat ho.
...
func void DIA_Bennet_GetInnosEye_Info ()
{
	...
	if (Bennet_RepairDay < Wld_GetDay())
	{
		...
		AI_Output			(self, other, "DIA_Bennet_GetInnosEye_06_03"); //Pracoval jsem na nęm celou noc a teë vypadá jako nový.
	...
	{	//Hack Mattes
		B_SayBennetLATER();
		//AI_Output			(self, other, "DIA_Bennet_GetInnosEye_06_04"); //Ne, ještę ne. Vraă se pozdęji.
		AI_Output			(self, other, "DIA_Bennet_GetInnosEye_06_05"); //Pokud mę budeš zdržovat, bude to trvat déle.
...
instance DIA_Bennet_DRACHENEIER		(C_INFO)
{
	...
	description	 = 	"Je nęco, co můžeš udęlat s dračími vejci?";
};
...
{
	AI_Output			(other, self, "DIA_Bennet_DRACHENEIER_15_00"); //Je nęco, co můžeš udęlat s dračími vejci?
	...
	AI_Output			(other, self, "DIA_Bennet_DRACHENEIER_15_02"); //Vzal jsem je ještęranům.
	...
	IF (DRACHENEIER_angebotenXP_OneTime == FALSE)
	{
	B_LogEntry (TOPIC_DRACHENEIER,"Bennet je ochoten mi dobâe zaplatit za všechna dračí vejce, která naleznu."); 
...
func void DIA_Bennet_DRACHENEIER_ok ()
{
	...
	AI_Output			(self, other, "DIA_Bennet_DRACHENEIER_ok_06_02"); //Tak tyhle vęcičky nos dál, pokud je budeš schopen dostat.
	...
	AI_Output			(self, other, "DIA_Bennet_DRACHENEIER_ok_06_03"); //Ehm. Âíkáš, že jsi je vzal ještęranům?
	AI_Output			(other, self, "DIA_Bennet_DRACHENEIER_ok_15_04"); //Správnę.
	AI_Output			(self, other, "DIA_Bennet_DRACHENEIER_ok_06_05"); //Z toho, co jsem slyšel, tak vętšina ještęranů bydlí v jeskyních.
	...
	B_LogEntry (TOPIC_DRACHENEIER,"Bennet si myslí, že bych se po vejcích męl podívat v khorinisských jeskyních, kde by męli ještęrani pâebývat."); 
	...
	{
		AI_Output			(self, other, "DIA_Bennet_DRACHENEIER_ok_06_08"); //Nejdâív ale musíš získat mapu jeskyní od zemępisce ve męstę. Byla by to škoda, kdybys je nemohl najít všechny.
...
func void DIA_Bennet_EierBringen_Info ()
{
	...
	{
		AI_Output			(self, other, "DIA_Bennet_EierBringen_06_05"); //Kdy tyhle vęcičky vůbec poâád nacházíš? Vsadím se, že už jich tady moc nebude.
	}
	else
	{
		AI_Output			(self, other, "DIA_Bennet_EierBringen_06_06"); //Nemyslím si, že teë nęjaké další najdeš. A já už jich mám tolik, že ani nevím, co s nimi budu dęlat.
...
instance DIA_Bennet_KnowWhereEnemy		(C_INFO)
{
	...
	PERMANENT 	 =  TRUE;
	description	 = 	"Potâebuju se dostat na ostrov. Kováâ by se mi mohl hodit.";
};
...
func void DIA_Bennet_KnowWhereEnemy_Info ()
{
	...
	AI_Output			(other, self, "DIA_Bennet_KnowWhereEnemy_15_02"); //Jo. Co na to âíkáš? V každém pâípadę by tę to dostalo odtud pryč.
	...
	B_LogEntry (TOPIC_Crew,"Bennet by mohl odejít klidnę hned teë. Jeho kováâskému umęní se hned tak nękdo nevyrovná a já se od nęj mohu lecos pâiučit.");
...
func void DIA_Bennet_LeaveMyShip_Info ()
{
	...
	AI_Output			(self, other, "DIA_Bennet_LeaveMyShip_06_01"); //Jednou je to takhle a za chvíli zase jinak. Dej si to v hlavę dohromady, jo? A až budeš vędęt, co vlastnę chceš, dej mi potom vędęt.
