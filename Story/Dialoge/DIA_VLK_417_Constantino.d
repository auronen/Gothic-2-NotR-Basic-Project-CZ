...
INSTANCE DIA_Constantino_Heilung   (C_INFO)
{
	...
	permanent   = FALSE;
	Description = "Potâebuji uzdravit.";
};
...
instance DIA_Addon_Constantino_LestersKraeuter		(C_INFO)
{
	...
	description	 = 	"Zajímá tę koupę nęjakých bylin?";
};
...
{
	AI_Output	(other, self, "DIA_Addon_Constantino_LestersKraeuter_15_00"); //Zajímá tę koupę nęjakých bylin?
	AI_Output	(self, other, "DIA_Addon_Constantino_LestersKraeuter_10_01"); //Pokud je tu nęco co můžeš nabídnout ...
...
func void DIA_Constantino_HerbsRunning_Success()
{
	...
	Log_CreateTopic(TOPIC_Lehrling,LOG_MISSION);
	Log_SetTopicStatus (TOPIC_Lehrling,LOG_RUNNING);
	B_LogEntry (Topic_Lehrling,"Constantino mę pâíjme do učení, jen když získám doporučení od čtyâech mistrů.");
...
FUNC VOID DIA_Constantino_LEHRLING_Info()
{
	...
	if (B_GetGreatestPetzCrime (self) == CRIME_NONE)
	{
		...
		{
			AI_Output (self, other,"DIA_Constantino_LEHRLING_10_05"); //Ale Harad tvrdí, že tę ještę nikdy nevidęl.
		};
		...
		{
			AI_Output (self, other,"DIA_Constantino_LEHRLING_10_19"); //Matteo tę prý ve svém krámku ještę nikdy nevidęl.
		};		
	...
	{
		AI_Output (self, other,"DIA_Constantino_LEHRLING_10_25"); //(podráždęnę) Ani náhodou! Slyšel jsem, žes byl tady v Khorinisu obžalován z nęjakého zločinu!
...
FUNC VOID DIA_Constantino_Aufgaben_Info()
{
	...
	AI_Output (self, other,"DIA_Constantino_Aufgaben_10_01"); //Nejsem tak naivní, abych si myslel, že mladík jako ty bude celý den trávit se mnou mezi čtyâmi stęnami.
...
FUNC VOID DIA_Constantino_Alchemy_Info()
{
	...
	AI_Output (self, other,"DIA_Constantino_Alchemy_10_04"); //OK pâípravę lektvaru s pomocí alchymistické kolony potâebuješ laboratorní baŕku.
	...
	B_LogEntry (TOPIC_CityTeacher,"Constantino naučí alchymistickému umęní.");
};
...
FUNC VOID DIA_Constantino_NewRecipes_Info()
{
	...
	AI_Output (self, other,"DIA_Constantino_NewRecipes_10_01"); //Máš s alchymií nęjaké pâedchozí zkušenosti?
	...
	Constantino_TeachAlchemy = TRUE;
	Log_CreateTopic (TOPIC_CityTeacher,LOG_NOTE);
	B_LogEntry (TOPIC_CityTeacher,"Constantino mę zasvętí do tajů alchymie.");
};
...
func void DIA_Constantino_TEACH_Health02()
{
	...
	{
		AI_Output (self, other,"DIA_Constantino_TEACH_Health02_10_00"); //OK pâípravę extraktu hojivé síly si musíš opatâit léčivé byliny a luční pohanku.
...
func void DIA_Constantino_TEACH_Mana02()
{
	if B_TeachPlayerTalentAlchemy (self, other, POTION_Mana_02)
	{
	 	...
	 	AI_Output (self, other,"DIA_Constantino_TEACH_Mana02_10_01"); //Pâi pâípravę tohoto elixíru stačí mít trošku citu a nechat jej jemnę pâejít varem. Jeho ingredience tvoâí ohnivé kopâivy a luční pohanka.
	};
	Info_ClearChoices (DIA_Constantino_Teach);
};
